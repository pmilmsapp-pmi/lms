...
import { fileURLToPath } from 'url'; // [FIX] Import helper URL

// Import Model (tanpa .js/.ts karena pakai tsx)
import { Blog } from '../models/Blog'; 
import { User } from '../models/User'; 

// [FIX] Definisikan __dirname secara manual untuk ES Module
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// Load .env dari root backend (naik 2 level dari src/scripts)
dotenv.config({ path: path.resolve(__dirname, '../../.env') });

// Gunakan URI dari .env Anda
...
const seedBlogs = async () => {
    try {
        ...
        // Debugging untuk memastikan URI terbaca
        ...
        if (!author) {
            ...
            console.error('   Pastikan Anda sudah register user dan mengubah rolenya di MongoDB Atlas.');
            process.exit(1);
        }
