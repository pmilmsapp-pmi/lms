...
import express, { Response } from 'express';
import { requireAuth, AuthedRequest } from '../middleware/auth';
import { Message } from '../models/Message'; // Import Model Message
import { 
    sendMessage, 
    getGlobalMessages, 
    getConversations, 
...
// [FITUR BARU] NUKE CHAT (Hapus Chat Error/Nyangkut)
// Hanya bisa diakses oleh Admin untuk membersihkan Global Chat
router.delete('/nuke', requireAuth, async (req: AuthedRequest, res: Response) => {
    try {
        if (!['SUPER_ADMIN', 'ADMIN'].includes(req.user!.role)) {
            return res.status(403).json({ error: "Hanya Admin yang bisa reset chat" });
        }
        await Message.deleteMany({}); // HAPUS SEMUA PESAN DI DB
        res.json({ message: '⚠️ SYSTEM RESET: Semua chat berhasil dihapus bersih.' });
    } catch (e: any) { 
        res.status(500).json({ error: e.message }); 
    }
...
// Specific User Chat (Taruh paling bawah)
router.get('/:otherUserId', requireAuth, getMessagesWithUser);
