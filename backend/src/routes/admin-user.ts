...
// // import { Certificate } from '../models/Certificate';
// // import { requireAuth, requireSuperAdmin, AuthedRequest } from '../middleware/auth';
// // // [PENTING] Import fungsi controller baru
// // import { getAllUsers, getDashboardStats } from '../controllers/userController'; 
...
// // // =========================================================================
// // // 0. GET DASHBOARD STATS (Total Asli tanpa Limit)
// // // =========================================================================
// // // Route ini diletakkan paling atas agar tidak tertukar dengan route '/:id'
// // router.get('/stats', requireAuth, requireSuperAdmin, getDashboardStats);

// // // 1. GET ALL USERS (Dengan Filter & Smart Search)
// // // Menggunakan controller 'getAllUsers' yang sudah support filter provinsi, kota, role, dll.
// // router.get('/', requireAuth, requireSuperAdmin, getAllUsers);
...
// import { Progress } from '../models/Progress'; 
// import { Certificate } from '../models/Certificate';
// import { requireAuth, requireSuperAdmin, AuthedRequest, requireAdmin } from '../middleware/auth';
...
// // [FIX] Gunakan 'requireAuth' SAJA agar Fasilitator bisa akses. 
// // Filter keamanan sudah ada di dalam controller 'getAllUsers' (User Biasa/Fasilitator dibatasi query-nya).
// router.get('/', requireAuth, getAllUsers);

// // 2. GET USER DETAILS (Hanya Super Admin/Admin)
// router.get('/:id/details', requireAuth, requireAdmin, async (req: AuthedRequest, res: Response) => {
//   try {
//     const { id } = req.params;
//     const user = await User.findById(id).select('-password');
//     if (!user) return res.status(404).json({ success: false, error: 'User tidak ditemukan' });
...
// // 3. POST CREATE USER (Hanya Super Admin)
// router.post('/', requireAuth, requireSuperAdmin, async (req, res) => {
//   try {
//     const data = createUserSchema.parse(req.body);
//     const existing = await User.findOne({ email: data.email });
...
// // 4. PATCH UPDATE USER (Hanya Admin)
...
// // 5. RESET PASSWORD (Hanya Super Admin)
// router.patch('/:id/reset-password', requireAuth, requireSuperAdmin, async (req, res) => {
//     try {
//         const { id } = req.params;
//         const DEFAULT_PASS = '123456'; 
...
// // 6. DELETE USER (Hanya Super Admin)
// router.delete('/:id', requireAuth, requireSuperAdmin, async (req: AuthedRequest, res) => {
//   try {
//     const targetUser = await User.findById(req.params.id);
//     if (!targetUser) return res.status(404).json({ success: false, error: 'User tidak ditemukan' });
...
// Import fungsi controller
// [FIX] Tambahkan 'getUserDetailsAdmin' ke import
import { 
    getAllUsers, 
    getDashboardStats, 
    getUserDetailsAdmin // <-- PENTING: Import controller baru ini
...
// 1. GET ALL USERS (LIST - HYBRID SEARCH)
// Controller ini sudah support pencarian ke Postgres
router.get('/', requireAuth, getAllUsers);

// 2. [UPDATE FINAL] GET USER DETAILS (SUPPORT PG_ID)
// Ganti handler inline lama dengan controller 'getUserDetailsAdmin'
router.get('/:id/details', requireAuth, requireAdmin, getUserDetailsAdmin);
...
router.patch('/:id', requireAuth, requireAdmin, async (req, res) => {
  try {
    // [FIX] Cek jika ID virtual (pg_) -> Tolak edit
    if (req.params.id.startsWith('pg_')) {
        return res.status(400).json({ success: false, error: 'User ini belum aktif di LMS. Silakan minta user login terlebih dahulu.' });
    }
...
router.patch('/:id/reset-password', requireAuth, requireSuperAdmin, async (req, res) => {
    try {
        ...
        // [FIX] Cegah reset password user virtual
        if (id.startsWith('pg_')) {
             return res.status(400).json({ success: false, error: 'User belum aktif. Tidak bisa reset password.' });
        }
...
router.delete('/:id', requireAuth, requireSuperAdmin, async (req: AuthedRequest, res) => {
  try {
    // [FIX] Cegah delete user virtual (karena tidak ada di mongo)
    if (req.params.id.startsWith('pg_')) {
         return res.status(400).json({ success: false, error: 'User ini hanya data view dari Pusat. Tidak bisa dihapus dari sini.' });
    }
