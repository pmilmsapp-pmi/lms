...
export const getProgress = async (req: AuthedRequest, res: Response) => {
  try {
    ...
    // Hitung ulang persentase (on-the-fly)
    let percent = 0;
    const course: any = await Course.findById(courseId);
    if (course && progress) {
        let total = 0;
...
// [BARU] FUNGSI SUBMIT QUIZ
export const submitQuiz = async (req: AuthedRequest, res: Response) => {
    try {
        const { courseId, lessonId, answers } = req.body; // answers = { '0': 1, '1': 0 }
        const userId = req.user?.id;

        if (!userId) return res.status(401).json({ error: "Unauthorized" });

        // 1. Cari Kunci Jawaban di Course
        const course: any = await Course.findById(courseId);
        if (!course) return res.status(404).json({ error: "Course not found" });

        let lesson: any = null;
        for (const m of course.modules) {
            const found = m.lessons.find((l: any) => l._id.toString() === lessonId);
        ...
        const quizAnswersArray: number[] = []; // Simpan dalam format array urut index

        lesson.questions?.forEach((q: any, idx: number) => {
            const userAns = answers[idx]; // Jawaban user untuk index soal ini
            quizAnswersArray[idx] = userAns !== undefined ? userAns : -1; // -1 jika tidak dijawab

            // Pastikan perbandingan tipe data aman (Number)
        ...
        // 3. Simpan ke Progress
        let progress = await Progress.findOne({ userId, courseId });
        if (!progress) progress = new Progress({ userId, courseId, completedLessons: [], lessonDetails: [] });

        const strLessonId = lessonId.toString();
        
        // Hapus data lama jika ada (agar update)
        progress.lessonDetails = progress.lessonDetails.filter((d: any) => d.lessonId.toString() !== strLessonId);
        ...
        progress.lessonDetails.push({
            lessonId,
            type: 'quiz',
            quizAnswers: quizAnswersArray, // Simpan jawaban user
            score: score,
            attempts: 1, // Bisa dikembangkan logic attempt-nya
            submittedAt: new Date()
        });

        // Selalu tandai 'completedLessons' agar tombol berubah jadi hijau/review mode
        // Walaupun nilainya jelek, statusnya "Sudah Dikerjakan"
        if (!progress.completedLessons.some((id: any) => id.toString() === strLessonId)) {
            progress.completedLessons.push(lessonId);
        }
