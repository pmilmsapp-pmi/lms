...
//   // [FIX] WAJIB ADA: Penanda bahwa Admin sudah menyetujui Info Dasar
...
//   // [FIX] FIELD PENTING AGAR GEAR TERBUKA
//   isInfoCompleted: { type: Boolean, default: false },
...
interface ILesson extends Document {
  ...
  url: string;
  duration: number;
  type: 'video' | 'quiz' | 'text' | 'document' | 'video_url' | 'upload_doc' | 'essay' | 'google_classroom' | 'pdf' | 'download_doc' | 'image' | 'slide' | 'poll' | 'virtual_class' | 'game_memory' | 'game_scavenger' | 'game_emoji' | 'flashcard' | 'embed' | 'audio';
  isPreview: boolean;
  isActive: boolean;
  isMandatory: boolean;
  videoUrl?: string;
...
export interface ICourse extends Document {
  ...
  proposalDocuments: { name: string; url: string; label?: string }[];
  ...
  regionCode?: string; 
  targetParticipants?: string[]; // [BARU] Filter unsur peserta
...
const CourseSchema: Schema = new Schema({
  ...
  regionCode: { type: String, default: 'national' },
  targetParticipants: { type: [String], default: [] } // [BARU]
