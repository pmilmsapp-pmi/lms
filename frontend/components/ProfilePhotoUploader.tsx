...
import { useState } from 'react';
import axios from 'axios'; // Gunakan Axios langsung agar kontrol header lebih mudah
...
export default function ProfilePhotoUploader({
  ...
  const uploadAndSave = async () => {
    ...
    setLoading(true);
    try {
      // 0. Ambil Token (Penting untuk menghindari 'Invalid Token')
      const token = localStorage.getItem('token');
      if (!token) {
        throw new Error("Sesi habis. Silakan login ulang.");
      }

      // 1. Upload File ke Server (Endpoint Generic Backend)
      const formData = new FormData();
      formData.append('file', file);
      
      // Menggunakan endpoint /api/upload sesuai backend/src/routes/upload.ts
      const uploadRes = await axios.post(`${API_BASE_URL}/api/upload`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
          'Authorization': `Bearer ${token}` // [FIX] Kirim token manual
        }
      });
      
      const responseData = uploadRes.data;
      const uploadedUrl = responseData.url || responseData.file?.path;

      if (!uploadedUrl) {
        throw new Error("Gagal mendapatkan URL gambar dari server.");
      }

      // 2. Simpan URL ke Profil User di Database
      // Menggunakan endpoint /api/users/me (PATCH) sesuai backend/src/routes/users.ts
      await api('/api/users/me', {
        method: 'PATCH',
      ...
      if (onSaved) onSaved(uploadedUrl);
      ...
      console.error(err);
      const msg = err.response?.data?.error || err.message || 'Terjadi kesalahan saat upload.';
