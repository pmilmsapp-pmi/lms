...
export default function CourseGroupChat({ 
    ...
    // --- [FIX NOTIFIKASI] SINKRONISASI TOTAL PESAN SERVER ---
    // Kita ambil total count dari server saat chat dibuka, bukan mengandalkan panjang array pesan yang mungkin terpotong pagination
    const syncReadStatus = async () => {
        if (currentUser?.id && isOpen) {
            try {
                // Ambil total pesan sebenarnya dari server
                const res = await api(`/api/chat/group/${courseId}/count`);
                const total = res.count || 0;
                
                // Simpan ke LocalStorage
                const key = `chat_read_count_${courseId}_${currentUser.id}`;
                localStorage.setItem(key, total.toString());
                
                // Trigger event agar Badge langsung hilang
                window.dispatchEvent(new Event('chat-badge-update'));
            } catch (e) {
                console.error("Gagal sync notifikasi", e);
            }
        }
    };
            ...
            setMessages(prev => {
                if (JSON.stringify(res) !== JSON.stringify(prev)) {
                    if (isOpen) {
                        ...
                        // Panggil sync setiap ada pesan baru masuk saat chat terbuka
                        syncReadStatus();
                    }
                    return res;
                }
    ...
    // Efek saat chat dibuka pertama kali
    useEffect(() => {
        if (isOpen) {
            syncReadStatus();
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (lastAt !== -1) {
            const query = val.substring(lastAt + 1, selStart);
            if (!query.includes(' ')) { setMentionQuery(query); setShowMentionPopup(true); return; }
        }
        setShowMentionPopup(false);
    };
    ...
    const onEmojiClick = (emojiData: any) => {
        const text = inputText;
        const newText = text ? text.slice(0, cursorPos) + emojiData.emoji + text.slice(cursorPos) : emojiData.emoji;
        setInputText(newText);
        setShowEmoji(false);
    };
    ...
    const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const url = res.url || res.file?.url || res.imageUrl;
            const isImg = file.type.startsWith('image/');
            setAttachment({ url, type: isImg ? 'image' : 'file', name: file.name });
        } catch (err: any) { alert("Gagal upload: " + err.message); } finally { setIsUploading(false); if (fileInputRef.current) fileInputRef.current.value = ''; }
    };
    ...
    const handleSend = async (e?: React.FormEvent) => {
        ...
        try {
            await api(`/api/courses/${courseId}/groups/send`, {
                method: 'POST',
                body: { text: inputText, type: activeTab, attachment: attachment }
            });
            setInputText('');
            setAttachment(null);
            loadMessages();
            // Force sync setelah kirim pesan
            syncReadStatus();
        } catch (error: any) { alert("Gagal kirim pesan: " + error.message); } finally { setIsSending(false); }
    };
    ...
    return (
        <div className={containerClasses} role="dialog" aria-modal="true" onClick={() => !isFloating && onClose && onClose()}>
            <div className={isFloating ? "w-full h-full flex flex-col md:flex-row" : "bg-white w-full max-w-6xl h-[85vh] rounded-2xl shadow-2xl flex overflow-hidden"} onClick={(e) => e.stopPropagation()}>
                ...
                <div className="flex-1 flex flex-col min-w-0 relative">
                    <div className="h-16 bg-red-900 text-white px-4 flex items-center justify-between shrink-0">
                        <div className="flex items-center gap-3">
                            ...
                            {isFacilitator && (
                                <div className="ml-4 flex bg-red-950/30 rounded-lg p-1">
                                    <button onClick={() => setActiveTab('public')} className={`px-3 py-1 rounded text-xs font-bold transition-all ${activeTab === 'public' ? 'bg-white text-red-900 shadow' : 'text-red-100 hover:bg-red-800'}`}>Kelas (Publik)</button>
                                    <button onClick={() => setActiveTab('internal')} className={`px-3 py-1 rounded text-xs font-bold transition-all ${activeTab === 'internal' ? 'bg-white text-red-900 shadow' : 'text-red-100 hover:bg-red-800'}`}>Fasilitator</button>
                                </div>
                            )}
                        </div>
                        <div className="flex items-center gap-2">
                            <button onClick={() => setShowMembers(!showMembers)} className="p-2 hover:bg-white/10 rounded-full md:hidden" aria-label="Toggle Anggota" title="Anggota"><Users size={20}/></button>
                            {isFloating ? (
                                <button onClick={() => onClose && onClose()} className="p-2 hover:bg-white/10 rounded-full" aria-label="Minimize Chat" title="Minimize"><Minimize2 size={20}/></button>
                            ) : (
                                <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-full" aria-label="Tutup Chat" title="Tutup"><X size={24}/></button>
                            )}
                        </div>
                    </div>
                            ...
                            messages.map((msg, idx) => {
                                ...
                                return (
                                    <div key={idx} className={`flex gap-3 max-w-[85%] ${isMe ? 'self-end flex-row-reverse' : 'self-start'}`}>
                                        ...
                                        <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                                            <div className="flex items-center gap-2 mb-1"><span className="text-[10px] font-bold text-gray-600">{msg.sender?.name || 'Unknown'}</span><span className="text-[9px] text-gray-400">{new Date(msg.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span></div>
                                            <div className={`px-4 py-2 rounded-2xl text-sm shadow-sm break-words relative group ${isMe ? 'bg-red-600 text-white rounded-tr-none' : 'bg-white text-gray-800 border border-gray-200 rounded-tl-none'}`}>
                                                {msg.attachment && (
                                                    <div className="mb-2 bg-black/10 p-1 rounded">
                                                        {msg.attachment.type === 'image' ? <img src={getImageUrl(msg.attachment.url)} alt="Att" className="max-w-[150px] rounded-lg border border-white/20" /> : <a href={getImageUrl(msg.attachment.url)} target="_blank" className={`flex items-center gap-2 p-2 rounded ${isMe ? 'bg-red-700' : 'bg-gray-100'} hover:opacity-80`}><FileText size={16}/> <span className="truncate max-w-[150px] underline">{msg.attachment.name || 'Dokumen'}</span></a>}
                                                    </div>
                                                )}
                                                {msg.message}
                                            </div>
                    ...
                    {attachment && (<div className="px-4 pt-2 bg-white border-t flex items-center gap-3"><div className="relative bg-gray-50 border rounded-lg p-2 flex items-center gap-2 pr-8">{attachment.type === 'image' ? <ImageIcon size={20} className="text-purple-600"/> : <FileText size={20} className="text-blue-600"/>}<span className="text-xs max-w-[200px] truncate font-medium text-gray-700">{attachment.name}</span><button onClick={() => setAttachment(null)} className="absolute top-1 right-1 hover:bg-gray-200 rounded-full p-0.5 text-gray-500" title="Hapus Lampiran" aria-label="Hapus Lampiran"><X size={14}/></button></div></div>)}
                    ...
                    <form onSubmit={handleSend} className="p-3 bg-white border-t border-gray-200 flex gap-2 items-end shrink-0">
                        ...
                        <div className="flex-1 relative">
                            <input className="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-red-500 focus:border-red-500 block p-3 outline-none transition-all" placeholder={isUploading ? "Mengupload..." : `Ketik pesan... (@ untuk mention)`} value={inputText} onChange={handleInputChange} disabled={isUploading} autoFocus={!isFloating} title="Input Pesan" aria-label="Input Pesan"/>
                        </div>
                        <button type="submit" disabled={(!inputText.trim() && !attachment) || isSending || isUploading} className="bg-red-700 text-white hover:bg-red-800 p-3 rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed mb-0.5" title="Kirim Pesan" aria-label="Kirim Pesan"><Send size={18} className={isSending ? 'animate-pulse' : ''} /></button>
                    </form>
                </div>

                {showMembers && (
                    <div className="w-64 bg-white border-l border-gray-200 flex flex-col shrink-0 md:flex hidden md:flex">
                        <div className="p-4 border-b bg-gray-50 font-bold text-gray-700 text-sm flex justify-between items-center"><span>ANGGOTA ({activeMembersList.length})</span></div>
                        <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
                            {activeMembersList.map((m: any, idx) => (
                                <div key={idx} className="flex items-center gap-3 p-2 hover:bg-gray-50 rounded-lg transition-colors cursor-default">
                                    <div className="relative">
                                        {m!.avatarUrl ? <img src={getImageUrl(m!.avatarUrl)} className="w-8 h-8 rounded-full object-cover border" alt={m!.name} /> : <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white ${m!.roleLabel === 'Fasilitator' ? 'bg-red-500' : 'bg-slate-400'}`}>{m!.name?.charAt(0) || '?'}</div>}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-medium text-gray-900 truncate">{m!.name}</p>
                                        <p className={`text-[10px] uppercase font-bold tracking-wider ${m!.roleLabel === 'Fasilitator' ? 'text-red-600' : 'text-gray-400'}`}>{m!.roleLabel}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
