...
import { 
  Bell, MessageCircle, Settings, User as UserIcon, LogOut, 
  Menu, X, BookOpen, MessageSquare, FileText, LayoutDashboard, ChevronDown,
  FolderOpen, Building2, MonitorPlay, Newspaper, Users, Grid, ShieldCheck
} from 'lucide-react'; 
...
export default function Header() {
  ...
  return (
    <>
      <header className="bg-white/80 backdrop-blur-md border-b border-gray-200/50 sticky top-0 z-[100] font-sans transition-all duration-300">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between relative">
          ...
          <div className="flex items-center gap-4 z-20">
            <button 
                onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
                className="lg:hidden p-2 text-gray-700 hover:text-red-600 hover:bg-gray-100 rounded-xl transition-colors focus:outline-none"
                aria-label="Toggle Menu"
            >
                {isMobileMenuOpen ? <X size={24}/> : <Menu size={24}/>}
            </button>
            ...
            <Link href="/" className="flex items-center gap-3 group">
              <img src="/pmi-logo.png" alt="Logo PMI" className="h-10 w-auto group-hover:scale-105 transition-transform duration-300" />
              {/* <img src="/humanis.png" alt="Humanis" className="h-8 w-auto object-contain hidden md:block" /> */}
            </Link>
          </div>

          {/* 2. CENTER: DESKTOP MENU (REDESIGNED - PMI RED) */}
          {user && (
            <nav className="hidden lg:flex items-center gap-2 absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2">
                ...
                <Link 
                    ...
                    ${isActive('/courses') 
                        ? 'bg-red-700 text-white shadow-md shadow-red-100' // [UBAH] Merah PMI
                        : 'text-gray-600 hover:bg-red-50 hover:text-red-700'}`}
                >
                    {/* Icon berubah jadi putih saat aktif */}
                    <Grid size={16} className={isActive('/courses') ? 'text-white' : 'text-gray-400'}/>
                    Katalog Kelas
                </Link>
                
                {/* MENU: Interaktif (Dropdown) */}
                <div className="relative group">
                  <button className={`px-5 py-2 rounded-full text-sm font-bold flex items-center gap-2 transition-all outline-none 
                    ${isActive('/forum') || isActive('/library') || isActive('/blog') 
                        ? 'bg-red-700 text-white shadow-md shadow-red-100' // [UBAH] Merah PMI
                        : 'text-gray-600 group-hover:text-red-700 group-hover:bg-red-50'}`}
                  >
                    Interaktif
                    <div className="relative transform scale-75"><ForumNotificationBadge /></div>
                    <ChevronDown size={14} className="mt-0.5 transition-transform duration-300 group-hover:rotate-180"/>
                  </button>
                  
                  {/* Dropdown Content */}
                  <div className="absolute top-full left-1/2 -translate-x-1/2 pt-4 w-72 invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all duration-300 ease-out z-50 transform group-hover:translate-y-0 translate-y-2">
                    <div className="bg-white/95 backdrop-blur-xl border border-gray-100 shadow-2xl rounded-2xl overflow-hidden p-2 ring-1 ring-black/5">
                      
                      <div className="px-4 py-2 text-[10px] font-black text-gray-400 uppercase tracking-widest">Kolaborasi</div>
                      
                      <Link href="/forum" className="flex items-center gap-4 px-4 py-3 text-sm font-bold text-gray-700 hover:bg-red-50 hover:text-red-700 rounded-xl transition-colors group/item">
                          <div className="p-2.5 bg-red-100 text-red-600 rounded-xl group-hover/item:bg-white group-hover/item:shadow-sm transition-all"><MessageSquare size={18}/></div>
                          <div className="flex-1">
                            <span className="block">Forum Diskusi</span>
                            <span className="text-[10px] font-normal text-gray-400 block">Tanya jawab & diskusi</span>
                          </div>
                          <div className="scale-75"><ForumNotificationBadge /></div>
                      </Link>
                      
                      <Link href="/library" className="flex items-center gap-4 px-4 py-3 text-sm font-bold text-gray-700 hover:bg-blue-50 hover:text-blue-700 rounded-xl transition-colors group/item">
                          <div className="p-2.5 bg-blue-100 text-blue-600 rounded-xl group-hover/item:bg-white group-hover/item:shadow-sm transition-all"><BookOpen size={18}/></div>
                          <div className="flex-1">
                            <span className="block">Perpustakaan</span>
                            <span className="text-[10px] font-normal text-gray-400 block">Materi & referensi</span>
                          </div>
                          <div className="scale-75"><LibraryNotificationBadge /></div>
                      </Link>
                      
                      <Link href="/blog" className="flex items-center gap-4 px-4 py-3 text-sm font-bold text-gray-700 hover:bg-green-50 hover:text-green-700 rounded-xl transition-colors group/item">
                          <div className="p-2.5 bg-green-100 text-green-600 rounded-xl group-hover/item:bg-white group-hover/item:shadow-sm transition-all"><FileText size={18}/></div>
                          <div className="flex-1">
                            <span className="block">Blog Relawan</span>
                            <span className="text-[10px] font-normal text-gray-400 block">Cerita & berita</span>
                          </div>
                          <div className="scale-75"><BlogNotificationBadge /></div>
                      </Link>
                    </div>
                ...
                {/* MENU: Pengelolaan (Dropdown - Admin/Fasilitator) */}
                {canManageContent && (
                  <div className="relative group">
                    <button className={`px-5 py-2 rounded-full text-sm font-bold flex items-center gap-2 transition-all outline-none 
                        ${isActive('/admin') 
                            ...
                            : 'text-gray-600 group-hover:text-red-700 group-hover:bg-red-50'}`}
                    >
                      Pengelolaan <ChevronDown size={14} className="mt-0.5 transition-transform duration-300 group-hover:rotate-180"/>
                    </button>
                    
                    <div className="absolute top-full left-1/2 -translate-x-1/2 pt-4 w-72 invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all duration-300 ease-out z-50 transform group-hover:translate-y-0 translate-y-2">
                      <div className="bg-white/95 backdrop-blur-xl border border-gray-100 shadow-2xl rounded-2xl overflow-hidden p-2 ring-1 ring-black/5">
                        
                        <Link href="/admin/courses?type=all" className="flex items-center gap-4 px-4 py-3 text-sm font-bold text-gray-700 hover:bg-orange-50 hover:text-orange-700 rounded-xl transition-colors group/item">
                            <div className="p-2.5 bg-orange-100 text-orange-600 rounded-xl group-hover/item:bg-white group-hover/item:shadow-sm transition-all"><FolderOpen size={18}/></div>
                            <span className="flex-1">Semua Program</span>
                        </Link>
                        
                        <div className="h-px bg-gray-100 my-1 mx-4"></div>

                        <Link href="/admin/courses?type=training" className="flex items-center gap-4 px-4 py-2.5 text-sm font-medium text-gray-600 hover:bg-purple-50 hover:text-purple-700 rounded-xl transition-colors">
                            <div className="p-1.5 bg-purple-100 text-purple-600 rounded-lg"><Building2 size={16}/></div>
                            <span className="flex-1">Admin Pelatihan</span>
                        </Link>

                        <Link href="/admin/courses?type=course" className="flex items-center gap-4 px-4 py-2.5 text-sm font-medium text-gray-600 hover:bg-cyan-50 hover:text-cyan-700 rounded-xl transition-colors">
                            <div className="p-1.5 bg-cyan-100 text-cyan-600 rounded-lg"><MonitorPlay size={16}/></div>
                            <span className="flex-1">Admin Kursus</span>
                        </Link>

                        <Link href="/admin/library" className="flex items-center gap-4 px-4 py-2.5 text-sm font-medium text-gray-600 hover:bg-yellow-50 hover:text-yellow-700 rounded-xl transition-colors">
                            <div className="p-1.5 bg-yellow-100 text-yellow-600 rounded-lg"><BookOpen size={16}/></div>
                            <span className="flex-1">Kelola Pustaka</span>
                        </Link>

                        <Link href="/admin/blog" className="flex items-center gap-4 px-4 py-2.5 text-sm font-medium text-gray-600 hover:bg-emerald-50 hover:text-emerald-700 rounded-xl transition-colors">
                            <div className="p-1.5 bg-emerald-100 text-emerald-600 rounded-lg"><Newspaper size={16}/></div>
                            <span className="flex-1">Kelola Blog</span>
                        </Link>

                        {isSuperAdmin && (
                            <Link href="/admin/users" className="flex items-center gap-4 px-4 py-2.5 text-sm font-medium text-gray-600 hover:bg-pink-50 hover:text-pink-700 rounded-xl transition-colors border-t border-gray-50 mt-1 pt-2">
                                <div className="p-1.5 bg-pink-100 text-pink-600 rounded-lg"><Users size={16}/></div>
                                <span className="flex-1">Admin Users</span>
                            </Link>
                        )}
                      </div>
              ...
              <>
                ...
                {isAdminOrSuper ? (
                    ...
                    <div className="relative" ref={notifRef}>
                      ...
                      {isNotifOpen && (
                          <div className="absolute right-0 mt-4 w-80 sm:w-96 bg-white border border-gray-100 rounded-2xl shadow-2xl z-50 overflow-hidden animate-in fade-in zoom-in-95 origin-top-right ring-1 ring-black/5">
                              <div className="px-5 py-4 border-b border-gray-50 flex justify-between items-center bg-white">
                                  <h3 className="font-bold text-gray-900">Notifikasi</h3>
                                  <Link href="/notifications" className="text-xs font-bold text-red-600 hover:text-red-800" onClick={() => handleMarkAsRead('', '/notifications')}>Tandai Semua</Link>
                              </div>
                ...
                <div className="relative" ref={profileRef}>
                  ...
                  >
                    <div className="relative">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-red-600 to-red-800 text-white flex items-center justify-center font-bold overflow-hidden shadow-md border-2 border-white ring-1 ring-gray-100">
                        {user.avatarUrl ? (<img src={getImageUrl(user.avatarUrl)} alt="Avatar" className="w-full h-full object-cover" />) : ( user.name?.charAt(0).toUpperCase() || 'U' )}
                        </div>
                        {!isAdminOrSuper && <div className="absolute -bottom-0.5 -right-0.5 pointer-events-none"><ChatNotificationBadge /></div>}
                    </div>
                  ...
                  {isProfileOpen && (
                    <div className="absolute right-0 mt-4 w-72 bg-white border border-gray-100 rounded-2xl shadow-2xl z-50 overflow-hidden animate-in fade-in zoom-in-95 origin-top-right ring-1 ring-black/5">
                        <div className="px-6 py-5 border-b border-gray-50 bg-gradient-to-br from-gray-50 to-white">
                            <p className="text-base font-bold text-gray-900 truncate">{user.name}</p>
                            <p className="text-xs text-gray-500 truncate mt-0.5 font-medium">{user.email}</p>
                            <div className="flex items-center gap-2 mt-3">
                                <p className="text-[10px] text-white font-bold uppercase tracking-wider bg-gray-900 px-2 py-0.5 rounded-md shadow-sm">{user.role.replace('_', ' ')}</p>
                                {isSuperAdmin && <ShieldCheck size={14} className="text-blue-500"/>}
                            </div>
                        </div>
                        <div className="p-2 space-y-1">
                          <Link href="/profile" className="flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-gray-700 hover:bg-gray-50 hover:text-red-600 rounded-xl transition-colors group" onClick={() => setIsProfileOpen(false)}>
                            <div className="p-1.5 bg-gray-100 rounded-lg text-gray-500 group-hover:bg-red-100 group-hover:text-red-600 transition-colors"><UserIcon size={16}/></div>
                            Profil Saya
                          </Link>
                          <button onClick={() => { setIsProfileOpen(false); setIsChatOpen(true); }} className="w-full text-left flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-gray-700 hover:bg-gray-50 hover:text-red-600 rounded-xl transition-colors group">
                            <div className="p-1.5 bg-gray-100 rounded-lg text-gray-500 group-hover:bg-red-100 group-hover:text-red-600 transition-colors"><MessageCircle size={16}/></div>
                            <span className="flex-1">Pesan Masuk</span><ChatNotificationBadge />
                          </button>
                          <div className="h-px bg-gray-100 my-1 mx-2"></div>
                          <button onClick={handleLogout} className="w-full text-left flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-red-600 hover:bg-red-50 rounded-xl transition-colors group">
                            <div className="p-1.5 bg-red-50 rounded-lg text-red-500 group-hover:bg-red-200 group-hover:text-red-700 transition-colors"><LogOut size={16}/></div>
                            Keluar
                          </button>
                        </div>
                    </div>
        ...
        {isMobileMenuOpen && (
            <div className="lg:hidden fixed top-16 left-0 w-full h-[calc(100vh-4rem)] bg-white z-[90] overflow-y-auto animate-in slide-in-from-top-5 duration-200 border-t border-gray-100">
                <div className="flex flex-col p-6 space-y-6 pb-20">
                    <div className="flex items-center gap-3 pb-4 border-b border-gray-100">
                        <div className="w-12 h-12 rounded-full bg-red-100 flex items-center justify-center text-red-600 font-bold border border-red-200 overflow-hidden shadow-sm">
                            {user?.avatarUrl ? <img src={getImageUrl(user.avatarUrl)} className="w-full h-full object-cover" alt={user.name || 'Avatar'} /> : user?.name?.charAt(0).toUpperCase()}
                        </div>
                        <div>
                            <p className="font-bold text-gray-900 text-lg">{user?.name}</p>
                            <p className="text-xs text-gray-500 capitalize font-medium">{user?.role?.toLowerCase().replace('_', ' ')}</p>
                        </div>
                    </div>

                    <div className="space-y-2">
                        <Link href="/courses" className="flex items-center gap-3 px-4 py-3 rounded-xl bg-gray-900 text-white font-bold shadow-md active:scale-95 transition-transform" onClick={() => setIsMobileMenuOpen(false)}>
                            <Grid size={20}/> Katalog Kelas
                        </Link>
                    </div>
                    
                    <div className="space-y-3">
                        <p className="text-xs font-black text-gray-400 uppercase tracking-widest px-2">Interaktif</p>
                        <Link href="/forum" className="flex items-center gap-3 px-4 py-3 rounded-xl bg-gray-50 text-gray-700 font-bold hover:bg-red-50 hover:text-red-600 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
                            <MessageSquare size={20} className="text-red-500"/> Forum Diskusi <ForumNotificationBadge />
                        </Link>
                        <Link href="/library" className="flex items-center gap-3 px-4 py-3 rounded-xl bg-gray-50 text-gray-700 font-bold hover:bg-blue-50 hover:text-blue-600 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
                            <BookOpen size={20} className="text-blue-500"/> Perpustakaan
                        </Link>
                        <Link href="/blog" className="flex items-center gap-3 px-4 py-3 rounded-xl bg-gray-50 text-gray-700 font-bold hover:bg-green-50 hover:text-green-600 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
                            <FileText size={20} className="text-green-500"/> Blog Relawan
                        </Link>
                    </div>
                    ...
                    {canManageContent && (
                        <div className="space-y-3 pt-2 border-t border-gray-100">
                            <p className="text-xs font-black text-gray-400 uppercase tracking-widest px-2 mt-4">Pengelolaan</p>
                            <Link href="/admin/courses?type=all" className="flex items-center gap-3 px-4 py-3 rounded-xl bg-gray-50 text-gray-700 font-bold hover:bg-orange-50 hover:text-orange-600 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
                                <FolderOpen size={20} className="text-orange-500"/> Semua Program
                            </Link>
                            <Link href="/admin/courses?type=training" className="flex items-center gap-3 px-4 py-3 rounded-xl bg-gray-50 text-gray-700 font-bold hover:bg-purple-50 hover:text-purple-600 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
                                <Building2 size={20} className="text-purple-500"/> Admin Pelatihan
                            </Link>
                            <Link href="/admin/courses?type=course" className="flex items-center gap-3 px-4 py-3 rounded-xl bg-gray-50 text-gray-700 font-bold hover:bg-cyan-50 hover:text-cyan-600 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
                                <MonitorPlay size={20} className="text-cyan-500"/> Admin Kursus
                            </Link>
                            <Link href="/admin/library" className="flex items-center gap-3 px-4 py-3 rounded-xl bg-gray-50 text-gray-700 font-bold hover:bg-yellow-50 hover:text-yellow-600 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
                                <BookOpen size={20} className="text-yellow-500"/> Kelola Pustaka
                            </Link>
                            <Link href="/admin/blog" className="flex items-center gap-3 px-4 py-3 rounded-xl bg-gray-50 text-gray-700 font-bold hover:bg-emerald-50 hover:text-emerald-600 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
                                <Newspaper size={20} className="text-emerald-500"/> Kelola Blog
                            </Link>
                            <Link href="/admin/dashboard" className="flex items-center gap-3 px-4 py-3 rounded-xl bg-gray-50 text-gray-700 font-bold hover:bg-gray-200 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
                                <LayoutDashboard size={20}/> Dashboard
                            </Link>
                        </div>
                    )}
                    ...
                    {user && (
                        <div className="pt-4 mt-auto">
                            <button onClick={handleLogout} className="w-full flex items-center justify-center gap-3 px-4 py-4 rounded-xl bg-red-50 text-red-600 font-bold hover:bg-red-100 transition-colors shadow-sm active:scale-95">
                                <LogOut size={20}/> Keluar Aplikasi
                            </button>
                        </div>
                    )}
