...
export default function CourseFormModal({ course, onClose, onSuccess, facilitators, currentUser }: CourseFormModalProps) {
    ...
    const populateForm = (data: any) => {
        ...
        setFormData({
            ...
            facilities: Array.isArray(data.facilities) ? data.facilities : [],
            facilitatorIds: Array.isArray(data.facilitatorIds) ? data.facilitatorIds.map((f:any) => (typeof f === 'object' && f !== null ? f._id : f)) : [],
            pics: initialPics, // [FIX] Gunakan logic gabungan diatas
    ...
    const handleAddPicFromSearch = (user: any) => {
        ...
        const newPic = {
            id: user._id || user.id, // [FIX] SIMPAN ID USER (PENTING UNTUK PERMISSION)
            name: user.name,
            pmiStatus: user.role, 
            email: user.email,
            avatarUrl: user.avatarUrl
    ...
    const handleFinalSubmit = async () => {
        ...
        try {
            const validPics = formData.pics.filter((p: any) => p.name && p.name.trim() !== '');
            // [FIX] Ekstrak ID PIC untuk disimpan ke field 'picIds' di database
            ...
            const payload = {
                ...
                // [FIX] Kirim kedua field: pics (tampilan/legacy) dan picIds (permission)
                pics: validPics,
                picIds: picIds, 
