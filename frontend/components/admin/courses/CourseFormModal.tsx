...
export default function CourseFormModal({ course, onClose, onSuccess, facilitators, currentUser }: CourseFormModalProps) {
    ...
    // [FIX] Kita butuh state 'allFacilitators' yang lebih lengkap (termasuk info wilayah)
    // facilitators prop mungkin hanya ringkasan, jadi kita akan gunakan allUsers untuk pencarian juga.
    const [allUsers, setAllUsers] = useState<any[]>([]);
    ...
    useEffect(() => {
        ...
        // Load All Users (Untuk pencarian PIC & Fasilitator yang lebih lengkap)
        api('/api/users').then(res => {
            const users = res.users || [];
            setAllUsers(users);
        }).catch(() => {
            // Fallback jika API gagal, gunakan props facilitators
    ...
    // [FIX] FILTER & PENCARIAN FASILITATOR (DARI ALL USERS)
    // Filter hanya user dengan role FACILITATOR / SUPER_ADMIN / ADMIN
    // Dan belum terpilih
    const availableFacilitators = allUsers.filter(u => {
        const isEligible = ['FACILITATOR', 'SUPER_ADMIN', 'ADMIN'].includes(u.role);
        const isNotSelected = !formData.facilitatorIds.includes(u.id || u._id);
        const matchesSearch = (u.name?.toLowerCase().includes(searchFacilitator.toLowerCase()) || u.email?.toLowerCase().includes(searchFacilitator.toLowerCase()));
        return isEligible && isNotSelected && matchesSearch;
    });

    // Helper untuk menampilkan asal user (kota/provinsi)
    ...
    // List yang sudah terpilih (untuk ditampilkan di atas)
    const selectedFacilitators = allUsers.filter(u => formData.facilitatorIds.includes(u.id || u._id));

    // Pencarian PIC (User umum juga bisa jadi PIC tambahan)
    const filteredPicCandidates = allUsers.filter(u => ((u.name || '').toLowerCase().includes(searchPic.toLowerCase()) || (u.email || '').toLowerCase().includes(searchPic.toLowerCase())) && !formData.pics.some((p: any) => p.email === u.email));
    ...
    return (
        ...
        >
            ...
            {/* TAB 5: TIM & PIC - DENGAN PENCARIAN USER LENGKAP */}
            {activeTab === 'team' && (
                <div className="max-w-4xl mx-auto space-y-8">
                    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                        <h3 className="font-bold text-gray-800 mb-2 flex items-center gap-2"><Users size={18}/> Pilih Tim Fasilitator</h3>
                        ...
                        <div className="relative">
                            <Search className="absolute left-3 top-2.5 text-gray-400" size={16} />
                            <input type="text" placeholder="Cari nama atau email fasilitator..." className="w-full pl-9 p-2.5 border rounded-lg text-sm bg-gray-50 focus:ring-2 focus:ring-blue-500 outline-none" value={searchFacilitator} onChange={(e) => setSearchFacilitator(e.target.value)} aria-label="Cari Fasilitator"/>
                                        ...
                                        <button key={user._id || user.id} type="button" onClick={() => handleAddPicFromSearch(user)} className="w-full text-left px-4 py-3 hover:bg-gray-50 border-b last:border-0 flex items-center justify-between group">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[10px] text-gray-400">{user.email} • {user.role}</p>
                                                </div>
                                            </div>
