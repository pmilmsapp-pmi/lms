...
// import { useState, useEffect } from 'react';
// import { Eye, Bell, CheckCircle, XCircle, Clock, Lock } from 'lucide-react';
// import { getImageUrl, api } from '@/lib/api';
// // Import fungsi permission yang baru
// import { canManageCourse } from '@/lib/permissionUtils';
...
// export default function ProposalTable({ courses, onApprove, onReject, onViewDetail, currentUser }: ProposalTableProps) {
//     const [unreadTopics, setUnreadTopics] = useState<Set<string>>(new Set());
//     // Variabel ini hanya untuk membedakan Fasilitator vs Admin biasa
//     const isAdminOrSuper = currentUser && ['ADMIN', 'SUPER_ADMIN'].includes(currentUser.role);
...
//             try {
//                 const res = await api('/api/notifications/unread-topics');
//                 if (res.topics) setUnreadTopics(new Set(res.topics));
//             } catch (e) { }
//         };
//         fetchUnreadTopics();
//         const interval = setInterval(fetchUnreadTopics, 5000);
...
//             await api('/api/notifications', {
//                 method: 'POST',
//                 body: { roleTarget: 'ADMIN', type: 'course', message: `📢 ${currentUser?.name} mengingatkan review untuk: "${course.title}".`, targetUrl: `/admin/courses?highlight=${course._id}`, topic: course._id }
//             });
//             alert("🔔 Pengingat terkirim!");
//         } catch (e: any) { alert("Gagal: " + e.message); }
//     };
...
//     const handleApproveWithNotif = async (course: any) => {
//         if(!confirm(`Setujui pelatihan "${course.title}"?`)) return;
//         try { await onApprove(course); } catch (e) { console.error(e); }
//     };
...
//             <tbody className="divide-y divide-gray-50 bg-white">
//                 {courses.map(course => {
//                     // CEK HAK AKSES DISINI
//                     const isAllowedToManage = canManageCourse(currentUser, course);
...
//                                     <Clock size={10}/> {course.status === 'revision' ? 'Menunggu Revisi' : 'Menunggu Review'}
//                                     {currentUser?._id === course.creatorInfo?._id && <span className="text-blue-500 ml-1">(Milik Saya)</span>}
//                                     {/* DEBUGGING: Uncomment baris ini jika ingin melihat regionCode di layar */}
//                                     {/* <span className='text-[8px] text-gray-400 block'>{course.regionCode}</span> */}
//                                 </div>
//                             </td>
//                             <td className="p-4">
//                                 <div className="flex items-center gap-3">
...
//                             <td className="p-4 text-center">
//                                 <div className="flex justify-center items-center gap-2">
//                                     <button onClick={() => onViewDetail(course)} className="relative p-2 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-600 transition-colors group" title="Lihat Detail">
//                                         <Eye size={16}/>
//                                         {unreadTopics.has(course._id) && <span className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 border-2 border-white rounded-full animate-pulse"></span>}
//                                     </button>

//                                     {!isAdminOrSuper ? (
//                                         <button onClick={() => handleRemindAdmin(course)} className="p-2 rounded-lg border border-orange-200 text-orange-600 hover:bg-orange-50 transition-colors" title="Ingatkan Admin"><Bell size={16}/></button>
//                                     ) : (
//                                         // LOGIKA TOMBOL DISINI
//                                         isAllowedToManage ? (
//                                             <>
//                                                 <button onClick={() => onReject(course)} className="p-2 rounded-lg border border-yellow-200 text-yellow-700 hover:bg-yellow-50 transition-colors" title="Kembalikan"><XCircle size={16}/></button>
//                                                 <button onClick={() => handleApproveWithNotif(course)} className="px-3 py-2 rounded-lg bg-green-600 text-white hover:bg-green-700 font-bold text-xs flex items-center gap-1 shadow-sm" title="Setujui"><CheckCircle size={14}/> Setujui</button>
//                                             </>
//                                         ) : (
//                                             <div className="flex items-center gap-1 px-2 py-1 bg-gray-50 border border-gray-200 rounded text-[10px] text-gray-400 cursor-not-allowed" title="Anda tidak memiliki akses kelola untuk wilayah ini">
//                                                 <Lock size={12}/> <span className="hidden md:inline">View Only</span>
//                                             </div>
//                                         )
//                                     )}
//                                 </div>
...
import { useState, useEffect } from 'react';
import { Eye, Bell, Clock, Lock, FileSignature, BookOpen, EyeOff } from 'lucide-react';
import { getImageUrl, api } from '@/lib/api';
// IMPORT FUNGSI PERMISSION (Pastikan path ini benar sesuai struktur folder Anda)
import { canVerifyCourse, canViewCourse } from '@/lib/permissionUtils';
...
export default function ProposalTable({ courses, onViewDetail, currentUser }: ProposalTableProps) {
    const [unreadTopics, setUnreadTopics] = useState<Set<string>>(new Set());
    const isAdminOrSuper = currentUser && ['ADMIN', 'SUPER_ADMIN'].includes(currentUser.role);
    ...
    const handleRemindAdmin = async (course: any, e: React.MouseEvent) => {
        e.stopPropagation();
        if(!confirm(`Kirim pengingat ke Admin?`)) return;
        try {
            await api('/api/notifications', {
        ...
        <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-50 bg-white">
                {courses.map(course => {
                    ...
                    // 1. Cek Hak Eksekusi (Review/Setujui)
                    // Syarat: Wilayah Cocok + Toggle "Verifikasi Pengajuan" ON
                    const canVerify = canVerifyCourse(currentUser, course);
                    
                    // 2. Cek Hak Lihat Detail (Mata)
                    // Syarat: Wilayah Cocok Saja
                    const canView = canViewCourse(currentUser, course);
                        ...
                        <tr key={course._id} className="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td className="p-4 text-center">
                                <div className="flex justify-center items-center gap-2">
                                    ...
                                    {/* LOGIKA FASILITATOR (Bukan Admin) */}
                                    {!isAdminOrSuper ? (
                                        <>
                                            <button onClick={() => onViewDetail(course)} className="p-2 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-600" title="Lihat"><Eye size={16}/></button>
                                            <button onClick={(e) => handleRemindAdmin(course, e)} className="p-2 rounded-lg border border-orange-200 text-orange-600 hover:bg-orange-50" title="Ingatkan Admin"><Bell size={16}/></button>
                                        </>
                                    ) : (
                                        // LOGIKA ADMIN
                                        canVerify ? (
                                            // A. PUNYA HAK REVIEW (Biru)
                                            <button 
                                                onClick={() => onViewDetail(course)} 
                                                className="relative px-4 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 font-bold text-xs flex items-center gap-2 shadow-sm transition-all" 
                                            ...
                                            // B. TIDAK PUNYA HAK REVIEW (View Only / Locked)
                                            <div className="flex gap-2">
                                                {canView ? (
                                                    // B1. Wilayah Cocok -> Boleh Lihat (Buku Terbuka)
                                                    <button 
                                                        onClick={() => onViewDetail(course)} 
                                                    ...
                                                    // B2. Wilayah Beda (Jabar lihat Jatim) -> Mata Mati (Dokumen Aman)
                                                    <button 
                                                        disabled 
                                                        className="p-2 rounded-lg bg-gray-50 border border-gray-200 text-gray-300 cursor-not-allowed" 
                                                        title="Dokumen Terkunci (Beda Wilayah)"
                                                    >
                                                        <EyeOff size={16}/>
                                                    </button>
                                                )}
                                                
                                                {/* Gembok Penanda Tidak Bisa Edit/Approve */}
                                                <div className="flex items-center justify-center p-2 bg-gray-50 border border-gray-200 rounded text-gray-400 cursor-not-allowed" title="Anda tidak memiliki izin verifikasi">
                                                    <Lock size={16}/>
                                                </div>
                                            </div>
                                        )
