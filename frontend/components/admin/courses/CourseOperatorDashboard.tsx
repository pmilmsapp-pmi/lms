...
// // Helper Fallback
// const BACKEND_URL = (typeof API_BASE_URL !== 'undefined' ? API_BASE_URL : 'http://localhost:4000').replace(/\/$/, '');

// // --- HELPER: MAPPING EMOJI ---
// const getLessonEmoji = (type: string) => {
//     switch (type) {
//         case 'video_url': return '🎞️';
//         case 'virtual_class': return '🎥';
//         case 'quiz': return '📝';
//         case 'essay': return '✍️';
//         case 'poll': return '📊';
//         case 'slide': return '🖥️';
//         case 'image': return '🖼️';
//         case 'upload_doc': return '📤';
//         case 'download_doc': return '📥';
//         case 'google_classroom': return '🏫';
//         case 'audio': return '🎧';
//         case 'embed': return '🌐';
//         case 'flashcard': return '🃏';
//         case 'game_memory': return '🎮';
//         case 'game_scavenger': return '📸';
//         case 'game_emoji': return '🤔';
//         default: return '📄';
//     }
// };
...
// // --- [FIX] DEEP URL EXTRACTOR ---
// const getSafeUrl = (input: any): string => {
//     if (!input) return '';
//     if (typeof input === 'string') {
//         const trimmed = input.trim().replace(/^"|"$/g, ''); 
//         if (trimmed.startsWith('http') || trimmed.startsWith('/') || trimmed.startsWith('/uploads')) return trimmed;
//         try { return getSafeUrl(JSON.parse(trimmed)); } catch (e) { return ''; }
//     }
//     if (typeof input === 'object' && input !== null) {
//         if (input.url && typeof input.url === 'string') return input.url;
//         if (input.secure_url && typeof input.secure_url === 'string') return input.secure_url;
//         if (input.fileUrl && typeof input.fileUrl === 'string') return input.fileUrl;
//         if (input.data) return getSafeUrl(input.data);
//         if (input.file) return getSafeUrl(input.file);
//         if (input.uploadedFile) return getSafeUrl(input.uploadedFile);
...
//                 </div>
//                 <div className="p-4 border-t bg-gray-50 flex justify-end">
//                     <button onClick={onClose} className="bg-gray-200 hover:bg-gray-300 text-gray-800 px-4 py-2 rounded-lg text-sm font-bold" title="Tutup" aria-label="Tutup">Tutup</button>
//                 </div>
//             </div>
//         </div>
//     );
...
//             <div className="flex border-b bg-gray-50 shrink-0">
//                 <button onClick={() => setActiveTab('progress')} className={`flex-1 py-3 text-sm font-bold text-center border-b-2 transition-colors ${activeTab === 'progress' ? 'border-indigo-600 text-indigo-700 bg-white' : 'border-transparent text-gray-500 hover:bg-gray-100'}`} title="Tab Progress" aria-label="Tab Progress">📊 Progress Belajar</button>
//                 <button onClick={() => setActiveTab('answers')} className={`flex-1 py-3 text-sm font-bold text-center border-b-2 transition-colors ${activeTab === 'answers' ? 'border-indigo-600 text-indigo-700 bg-white' : 'border-transparent text-gray-500 hover:bg-gray-100'}`} title="Tab Jawaban" aria-label="Tab Jawaban">📝 Jawaban</button>
//             </div>
...
//                                         <div key={l._id} className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
//                                             <div className="flex justify-between items-start mb-3 border-b pb-2">
//                                                 <div><h5 className="font-bold text-gray-800 text-sm">{l.title}</h5><span className="text-[10px] bg-blue-50 text-blue-600 px-2 py-0.5 rounded uppercase font-bold flex items-center gap-1 mt-1 w-fit">{getLessonEmoji(l.type)} {l.type.replace('game_', 'Game ')}</span></div>
//                                                 <span className="text-[10px] text-gray-400">
//                                                     {(answerData?.submittedAt || quizData?.submittedAt) 
//                                                         ? new Date(answerData?.submittedAt || quizData?.submittedAt).toLocaleString('id-ID') 
//                                                         : 'Belum dikerjakan'}
...
//                                                                         <div className="flex-1 overflow-hidden"><p className="font-bold truncate text-sm">Dokumen Jawaban</p></div>
//                                                                         {safeUrl ? (
//                                                                             <a href={getFileUrl(safeUrl)} target="_blank" rel="noopener noreferrer" className="bg-blue-600 text-white px-3 py-1.5 rounded text-xs font-bold flex items-center gap-1 hover:bg-blue-700 transition-colors" title="Lihat/Download File" aria-label="Download File">
//                                                                                 <ExternalLink size={14}/> Buka File
//                                                                             </a>
//                                                                         ) : (
//                                                                             <div className="flex flex-col">
...
//         m.lessons?.forEach((l: any) => {
//           let label = l.title;
//           const emoji = getLessonEmoji(l.type);
//           label = `${emoji} ${l.title}`;
//           opts.push({ id: l._id, label, type: l.type });
//         });
//       });
...
//           }));
//           setParticipants(hydratedData); 
//           // [FIX] Panggil kalkulasi poll stats setelah data hydrated
//           calculatePollStats(hydratedData);
//       } else {
//           calculatePollStats([]);
//       }
...
//       {/* [FIX] HASIL POLLING DIKEMBALIKAN */}
...
//                 <option value="">-- Pilih Materi / Kuis --</option>
//                 {actionOptions.map(opt => {
//                     return <option key={opt.id} value={opt.id}>{getLessonEmoji(opt.type)} {opt.label}</option>
//                 })}
//             </select>
//             <input className="px-3 py-1.5 rounded border text-xs h-9 outline-none min-w-[150px]" placeholder="Cari Nama..." value={participantFilter} onChange={e => setParticipantFilter(e.target.value)} title="Cari Peserta" aria-label="Cari Peserta"/>
//           </div>
...
// --- HELPER: MAPPING EMOJI ---
// Emoji digunakan karena tag <option> tidak mendukung komponen Icon SVG
const getLessonEmoji = (type: string) => {
    switch (type) {
        case 'video_url': return '🎞️';
        case 'virtual_class': return '🎥';
      ...
      <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
        <div className="bg-gray-50 px-6 py-4 border-b flex justify-between items-center flex-wrap gap-4">
          ...
          <div className="flex gap-2 items-center bg-white p-1 rounded border shadow-sm">
            ...
            <select id="action-select" className="p-1.5 border rounded text-xs bg-gray-50 h-9 outline-none min-w-[200px]" value={selectedActionId} onChange={e => setSelectedActionId(e.target.value)} title="Pilih Materi" aria-label="Pilih Materi">
                <option value="">-- Pilih Materi / Kuis --</option>
                {/* [FIX] GUNAKAN EMOJI AGAR TIDAK DOUBLE ICON */}
                {actionOptions.map(opt => {
