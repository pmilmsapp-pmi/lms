...
// import { useState, useRef, useEffect } from 'react';
// import { FileText, CheckCircle, Undo2, MessageSquare, Loader2, Send, Upload, ExternalLink, Building, MapPin, X, Users } from 'lucide-react';
...
//     // Ambil target peserta
//     const targetParticipants = course.targetParticipants || [];
...
//                                     {/* [BARU] UNSUR PESERTA DI FORM REVIEW */}
//                                     <div className="pt-4 border-t border-gray-100">
//                                         <label className="text-[10px] font-black text-gray-400 uppercase block mb-2 flex items-center gap-1"><Users size={12}/> Unsur Peserta</label>
//                                         <div className="flex flex-wrap gap-2">
//                                             {targetParticipants.length > 0 ? (
//                                                 targetParticipants.map((tp: string) => (
//                                                     <span key={tp} className="px-2.5 py-1 bg-gray-100 border border-gray-200 rounded-lg text-xs font-bold text-gray-700">
//                                                         {tp}
//                                                     </span>
//                                                 ))
//                                             ) : (
//                                                 <span className="text-xs text-gray-400 italic">Semua Unsur Anggota (Tidak Spesifik)</span>
//                                             )}
//                                         </div>
//                                     </div>
//                                 </div>
...
import { useState, useRef, useEffect } from 'react';
// [FIX] Menambahkan 'Lock' ke dalam import lucide-react
import { FileText, CheckCircle, Undo2, MessageSquare, Loader2, Send, Upload, ExternalLink, Building, MapPin, X, Users, UserCircle, LayoutGrid, Info, Lock } from 'lucide-react';
import { api } from '@/lib/api'; 
import axios from 'axios';
import BaseModal from '@/components/ui/BaseModal';
import { canAccessCourse, canVerifyCourse } from '@/lib/permissionUtils'; 
...
export default function CourseProposalDetailModal({ 
    ...
    return (
        <BaseModal isOpen={true} onClose={onClose} title="Review Detail Usulan" size="full">
            <div className="max-w-6xl mx-auto flex flex-col h-[600px] -m-1">
                <div className="flex border-b border-gray-200 px-4 bg-white shrink-0">
                    <button onClick={() => setActiveTab('detail')} className={`flex items-center gap-2 px-6 py-4 text-sm font-bold border-b-2 transition-colors ${activeTab === 'detail' ? 'border-red-600 text-red-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}><FileText size={16}/> Detail & Dokumen</button>
                    <button onClick={() => setActiveTab('feedback')} className={`flex items-center gap-2 px-6 py-4 text-sm font-bold border-b-2 transition-colors ${activeTab === 'feedback' ? 'border-red-600 text-red-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}><MessageSquare size={16}/> Diskusi & Revisi {localHistory.length > 0 && <span className="bg-red-100 text-red-600 px-2 py-0.5 rounded-full text-[10px]">{localHistory.length}</span>}</button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto bg-gray-50 p-6 custom-scrollbar">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-6">
                                <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-500 to-blue-700 text-white flex items-center justify-center font-bold text-lg shadow-md">{creator.name?.charAt(0)}</div>
                                    <div>
                                        <div className="flex items-center gap-2 mb-1">
                                            <span className="text-[10px] font-black text-blue-600 uppercase tracking-wide bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100">PENGAJU</span>
                                            <span className="text-[10px] font-bold text-gray-500 uppercase">{creator.role || 'Fasilitator'}</span>
                                        </div>
                                        <p className="text-base font-bold text-gray-900 leading-tight">{creator.name}</p>
                                        <p className="text-xs text-gray-500">{creator.email}</p>
                                    </div>
                                </div>
                                ...
                                <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                                    <div className="bg-gray-50 px-5 py-3 border-b border-gray-200">
                                        <h3 className="font-bold text-gray-800 text-sm flex items-center gap-2"><Building size={16}/> Informasi Pelatihan</h3>
                                    </div>
                                    <div className="p-5 space-y-5">
                                        ...
                                        <div className="flex flex-wrap gap-3">
                                            <div className="bg-gray-50 px-3 py-2 rounded-lg border border-gray-200">
                                                <span className="text-[10px] text-gray-400 uppercase font-bold block mb-0.5">TIPE</span>
                                                <div className="flex items-center gap-1 font-bold text-gray-700 text-sm">
                                                    {course.programType === 'training' ? <LayoutGrid size={14}/> : <Building size={14}/>}
                                                    {course.programType === 'training' ? 'Diklat Resmi' : 'Kursus Mandiri'}
                                                </div>
                                            </div>
                                            <div className="bg-gray-50 px-3 py-2 rounded-lg border border-gray-200">
                                                <span className="text-[10px] text-gray-400 uppercase font-bold block mb-0.5">LOKASI PELAKSANA</span>
                                                <div className="flex items-center gap-1 font-bold text-gray-700 text-sm">
                                                    <MapPin size={14} className="text-red-500"/>
                                                    {course.organizer || 'Nasional'}
                                                </div>
                                            </div>
                                        </div>

                                        <div className="pt-4 border-t border-dashed border-gray-200">
                                            <label className="text-[10px] font-black text-gray-400 uppercase block mb-2 flex items-center gap-1">
                                                <UserCircle size={12}/> Target Peserta (Jabatan)
                                            </label>
                                            <div className="flex flex-wrap gap-2">
                                                {targetParticipants.length > 0 ? (
                                                    targetParticipants.map((tp: string, idx: number) => (
                                                        <span key={idx} className="px-3 py-1 bg-blue-50 border border-blue-100 text-blue-700 rounded-full text-xs font-bold shadow-sm">
                                                            {tp}
                                                        </span>
                                                    ))
                                                ) : (
                                                    <span className="text-xs text-gray-400 italic bg-gray-50 px-3 py-1 rounded-full border border-gray-100">
                                                        Semua Unsur Anggota (Umum)
                                                    </span>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        <div>
                                            <label className="text-[10px] font-black text-gray-400 uppercase block mb-1">LATAR BELAKANG</label>
                                            <div className="text-sm text-gray-700 leading-relaxed bg-gray-50/50 p-3 rounded-lg border border-gray-100" dangerouslySetInnerHTML={{ __html: course.description || '-' }} />
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-white rounded-xl border border-gray-200 shadow-sm h-fit">
                                <div className="bg-gray-50 px-5 py-3 border-b border-gray-200 flex justify-between items-center">
                                    <h3 className="font-bold text-gray-800 text-sm flex items-center gap-2"><FileText size={16}/> Dokumen Lampiran</h3>
                                    <span className="text-xs font-bold bg-white border px-2 py-0.5 rounded text-gray-500">{localDocuments.length} File</span>
                                </div>
                                <div className="p-5">
                                    {showActionButtons && (
                                        <div className="mb-4">
                                            <div className="relative group">
                                                <input type="file" onChange={handleUploadSusulan} disabled={uploading} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" title="Upload Dokumen Susulan" />
                                                <button className="w-full py-2 bg-blue-50 text-blue-600 rounded-lg text-xs font-bold border border-blue-200 border-dashed flex justify-center gap-2 items-center group-hover:bg-blue-100 transition-colors">
                                                    {uploading ? <Loader2 size={14} className="animate-spin"/> : <Upload size={14}/>} 
                                                    Upload Dokumen Susulan (Revisi)
                                                </button>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    <div className="space-y-3">
                                        {hasViewAccess ? localDocuments.map((doc: any, i: number) => (
                                            <div key={i} className="flex items-center justify-between p-3 bg-white hover:bg-gray-50 rounded-xl border border-gray-200 transition-all shadow-sm group">
                                                <div className="flex items-center gap-3 overflow-hidden">
                                                    <div className="p-2.5 bg-red-50 text-red-600 rounded-lg border border-red-100 group-hover:bg-red-100 transition-colors">
                                                        <FileText size={18}/>
                                                    </div>
                                                    <div className="flex flex-col">
                                                        <span className="text-xs font-bold text-gray-800 truncate w-48" title={doc.name}>{doc.label || doc.name}</span>
                                                        <span className="text-[10px] text-gray-400 truncate w-48">{doc.name}</span>
                                                    </div>
                                                </div>
                                                <a href={doc.url} target="_blank" className="text-blue-600 hover:text-blue-800 text-[10px] font-bold flex items-center gap-1 bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100 hover:shadow-md transition-all">
                                            ...
                                            <div className="text-center py-8 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                                                ...
                                                <p className="text-xs text-gray-500 font-medium">Dokumen terkunci.</p>
                                                <p className="text-[10px] text-gray-400">Anda tidak memiliki akses wilayah ke dokumen ini.</p>
                                            </div>
                                        )}
                                        
                                        {hasViewAccess && localDocuments.length === 0 && (
                                            <div className="text-center py-6 text-gray-400 text-xs italic">Belum ada dokumen yang diunggah.</div>
                                        )}
                                    </div>
                                </div>
                            </div>
                    ...
                    {activeTab === 'feedback' && (
                        <div className="flex flex-col h-full bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                            <div className="bg-gray-50 px-4 py-3 border-b border-gray-200 text-xs font-bold text-gray-500 uppercase tracking-wide text-center">
                                Riwayat Diskusi & Catatan Revisi
                            </div>
                            <div ref={chatContainerRef} className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-gray-50/50">
                                {localHistory.length === 0 && (
                                ...
                                {localHistory.map((msg: any, idx: number) => {
                                    ...
                                    if(msg.role === 'SYSTEM') return (
                                        <div key={idx} className="flex justify-center my-4">
                                            <span className="text-[10px] bg-gray-200 text-gray-600 px-3 py-1 rounded-full border border-gray-300 font-medium shadow-sm flex items-center gap-1">
                                                <Info size={10}/> {msg.message}
                                            </span>
                                        </div>
                                    );
                                    return (
                                        <div key={idx} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} animate-in slide-in-from-bottom-1`}>
                                            <div className={`px-4 py-2.5 rounded-2xl text-xs max-w-[85%] shadow-sm leading-relaxed ${isMe ? 'bg-blue-600 text-white rounded-br-none' : 'bg-white text-gray-800 rounded-bl-none border border-gray-200'}`}>
                                                {msg.message}
                                            </div>
                                            <span className="text-[9px] text-gray-400 mt-1 px-1 font-medium">{msg.senderName} • {new Date(msg.createdAt).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</span>
                                        </div>
                                    )
                                })}
                            </div>
                            {hasViewAccess && (
                                <div className="p-3 border-t bg-white flex gap-2">
                                    <input 
                                        className="flex-1 border border-gray-300 rounded-xl px-4 py-2.5 text-xs focus:ring-2 focus:ring-blue-500 outline-none transition-all" 
                                        placeholder="Ketik revisi, saran, atau komentar..." 
                                        value={newMessage} 
                                        onChange={e=>setNewMessage(e.target.value)} 
                                        onKeyDown={e=>e.key==='Enter' && handleSendFeedback()}
                                    />
                                    <button 
                                        ...
                                        className="bg-blue-600 hover:bg-blue-700 text-white p-2.5 rounded-xl transition-all shadow-md disabled:opacity-50 disabled:shadow-none" 
                                        title="Kirim Pesan"
                                    >
                                        {sending ? <Loader2 size={18} className="animate-spin"/> : <Send size={18}/>}
                                    </button>
                ...
                <div className="flex gap-3 pt-3 border-t border-gray-200 mt-auto bg-white p-4 items-center">
                    <button onClick={onClose} className="px-5 py-2.5 border border-gray-300 hover:bg-gray-100 rounded-xl text-xs font-bold text-gray-700 transition-colors" title="Tutup">Tutup</button>
                        ...
                        {showActionButtons ? (
                            <>
                                <button 
                                    onClick={handleRejectAction} 
                                    disabled={loadingAction} 
                                    className="px-5 py-2.5 bg-white text-red-600 border border-red-200 hover:bg-red-50 rounded-xl text-xs font-bold flex gap-2 items-center transition-colors shadow-sm"
                                >
                                    <Undo2 size={16}/> Minta Revisi / Tolak
                                </button>
                                <button 
                                    onClick={handleApproveAction} 
                                    disabled={loadingAction} 
                                    className="px-6 py-2.5 bg-green-600 text-white hover:bg-green-700 rounded-xl text-xs font-bold flex gap-2 items-center shadow-md hover:shadow-lg transition-all active:scale-95"
                                >
                                    {loadingAction ? <Loader2 size={16} className="animate-spin"/> : <CheckCircle size={16}/>} 
                                    Setujui Usulan
                                </button>
                            ...
                            hasViewAccess && <span className="flex items-center text-xs text-gray-500 italic px-3 py-1 bg-gray-100 rounded-lg border border-gray-200">Mode Baca Saja (Read Only)</span>
                        )}
                    </div>
                </div>
            </div>
