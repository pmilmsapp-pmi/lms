...
import {
    ...
    MonitorPlay, UploadCloud, GripVertical, Pencil, MessageSquare,
    Rocket, ShieldCheck, X, Bell, CheckCircle,
    Mic, Globe, Zap, Layers, Trash, Gamepad2, Camera, Smile, Check
} from 'lucide-react';
...
export default function CourseContentEditor({ course, courseId, refreshData, facilitators }: CourseContentEditorProps) {
    ...
    // [UPDATE] QUIZ LOGIC
    const addQuestionRow = () => setQuestions([...questions, { question: '', options: ['', '', '', ''], correctIndex: 0 }]);
    const updateQuestion = (idx: number, field: string, value: any, optIdx?: number) => {
    ...
    const addEssayRow = () => setEssayQuestions([...essayQuestions, '']);
    const updateEssay = (idx: number, val: string) => { setEssayQuestions(prev => { const newQ = [...prev]; newQ[idx] = val; return newQ; }); };
    const removeEssay = (idx: number) => { if (essayQuestions.length > 1) setEssayQuestions(essayQuestions.filter((_, i) => i !== idx)); };
    ...
    const handleSaveModule = async () => {
        ...
        try {
            ...
            resetModuleForm();
            refreshData();
        } catch (e: any) { alert(e.message); } finally { setIsSavingContent(false); }
    };
    ...
    const handleSaveLesson = async (modId: string) => {
        ...
        if (selectedFile) {
            try {
                ...
                fd.append('file', selectedFile);
                const res = await apiUpload('/api/upload', fd);
                fileUrl = res.url || res.file?.url || res.imageUrl || res.data?.url; // Fix URL extraction
            } catch (err) { alert("Gagal upload file!"); setIsSavingContent(false); return; } finally { setUploading(false); }
        } else if (editingLesId) { fileUrl = existingFileUrl || ''; }
        ...
        const newLessonData: any = {
            ...
            jp: lesJp, facilitatorId: lesFacilitatorId || null, scheduleDate: lesSchedule || null,
            quizDuration: quizDuration,
            // [UPDATE] Pastikan questions map ke correctIndex (sekarang pakai answerIndex biar sinkron backend)
            questions: (lesType === 'quiz') ? questions.map(q => ({
                question: q.question,
                options: q.options,
                answerIndex: q.correctIndex, // Sinkronisasi nama field (frontend: correctIndex -> backend: answerIndex)
                correctAnswer: q.correctIndex // Cadangan
            })) : undefined,
            pollQuestion: (lesType === 'poll') ? pollQuestion : undefined,
            pollOptions: (lesType === 'poll') ? pollOptions.filter(o => o.trim() !== '') : undefined,
            classroomData: (lesType === 'google_classroom' && selectedClassroom) ? selectedClassroom : undefined
        ...
        if (lesType === 'essay' || lesType === 'game_emoji') { newLessonData.questions = essayQuestions.map(q => ({ question: q, options: [], answerIndex: 0 })); newLessonData.content = lesContent; }
        if (lesType === 'flashcard') { newLessonData.content = JSON.stringify(flashcards); }
        if (lesType === 'game_memory') { newLessonData.content = JSON.stringify(memoryPairs); }
        ...
        try {
            if (editingLesId) {
                ...
                if (modIdx > -1) {
                    const lesIdx = updatedModules[modIdx].lessons.findIndex((l: any) => l._id === editingLesId);
                    if (lesIdx > -1) {
                        updatedModules[modIdx].lessons[lesIdx] = { ...updatedModules[modIdx].lessons[lesIdx], ...newLessonData };
                        await api(`/api/courses/${courseId}`, { method: 'PATCH', body: { modules: updatedModules } });
                    }
                }
            } else {
            ...
            resetLessonForm();
            refreshData();
        } catch (e: any) { alert("Gagal menyimpan: " + e.message); } finally { setIsSavingContent(false); }
    };
    ...
    const startEditLesson = (modId: string, les: any) => {
        ...
        if (contentType === 'quiz') {
            // MAPPING DATA DARI DB KE EDITOR
            // Backend pakai `answerIndex` atau `correctAnswer`. Kita mapping ke `correctIndex`.
            const mappedQuestions = (les.questions || []).map((q: any) => ({
                question: q.question,
                options: q.options,
                correctIndex: q.answerIndex !== undefined ? q.answerIndex : (q.correctAnswer || 0)
            }));
            setQuestions(mappedQuestions.length ? mappedQuestions : [{ question: '', options: ['', '', '', ''], correctIndex: 0 }]);
        }
        if (contentType === 'poll') { setPollQuestion(les.pollQuestion || ''); setPollOptions(les.pollOptions?.length ? les.pollOptions : ['', '']); }
        if (contentType === 'google_classroom') { const token = localStorage.getItem('google_class_token'); if (token) { setGoogleToken(token); fetchClassroomCourses(token); } if (les.classroomData) setSelectedClassroom(les.classroomData); } else { setSelectedClassroom(null); }
                                                                            ...
                                                                            <div id={`form-${m._id}`} className="bg-blue-50 p-4 rounded border border-blue-200 mt-2 space-y-3">
                                                                                ...
                                                                                {lesType === 'quiz' && (
                                                                                    <div className="bg-white p-3 rounded border">
                                                                                        {questions.map((q, ix) => (
                                                                                            <div key={ix} className="p-3 border rounded bg-gray-50 mb-3 relative">
                                                                                                <button type="button" onClick={() => removeQuestion(ix)} className="absolute top-2 right-2 text-red-400 hover:text-red-600 font-bold" title="Hapus Soal" aria-label="Hapus Soal"><Trash2 size={14} /></button>
                                                                                                <label className="text-xs font-bold text-gray-600 block mb-1">Pertanyaan #{ix + 1}</label>
                                                                                                <input className="w-full p-2 border rounded mb-3 text-sm" placeholder="Tulis pertanyaan..." value={q.question} onChange={e => updateQuestion(ix, 'question', e.target.value)} aria-label={`Pertanyaan ${ix + 1}`} />

                                                                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                                                                    {q.options.map((o: string, oi: number) => (
                                                                                                        <div key={oi} className={`flex items-center gap-2 p-2 border rounded transition-colors ${q.correctIndex === oi ? 'bg-green-50 border-green-300' : 'bg-white border-gray-200'}`}>
                                                                                                            <div className="flex items-center justify-center shrink-0">
                                                                                                                <input
                                                                                                                    type="radio"
                                                                                                                    name={`correct-answer-${ix}`}
                                                                                                                    checked={q.correctIndex === oi}
                                                                                                                    onChange={() => updateQuestion(ix, 'correctIndex', oi)}
                                                                                                            ...
                                                                                                            <input
                                                                                                                ...
                                                                                                                value={o}
                                                                                                                onChange={e => updateQuestion(ix, 'options', e.target.value, oi)}
                                                                                                                aria-label={`Opsi ${oi + 1}`}
                                                                                                            />
                                                                                                            {q.correctIndex === oi && <Check size={14} className="text-green-600" />}
                                                                                                        </div>
                                                                                                    ))}
                                                                                                </div>
                                                                                            </div>
                                                                                        ))}
                                                                                        <button type="button" onClick={addQuestionRow} className="text-xs bg-blue-50 text-blue-600 border border-blue-200 px-3 py-1.5 rounded hover:bg-blue-100 font-bold flex items-center gap-1" title="Tambah Soal" aria-label="Tambah Soal"><Plus size={14} /> Tambah Soal</button>
                                                                                    </div>
                                                                                )}
