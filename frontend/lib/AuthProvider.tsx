...
// // [UPDATE] Definisi Tipe User Lengkap (Hybrid Auth: MongoDB + Postgres Data)
// export interface User {
//   // --- Core Identity ---
//   id: string;
...
//   email: string;
//   role: 'STUDENT' | 'FACILITATOR' | 'ADMIN' | 'SUPER_ADMIN';
//   avatarUrl?: string; // URL Foto (bisa dari MongoDB atau sync path_foto Postgres)
...
//   // --- Admin Region Scope ---
//   regionScope?: string; // 'national', 'province', 'regency'
//   managedProvinces?: string[];
//   managedRegencies?: string[];

//   // --- Data Lokasi & Member Type (Legacy/Local) ---
//   city?: string;
//   province?: string;
//   memberType?: string; // KSR, TSR, PMR, Pegawai, Pengurus
//   status?: string;
  
//   // --- [DATA INTEGRASI POSTGRESQL] ---
//   // Field ini terisi otomatis saat login jika user berasal dari Database Pusat
//   memberData?: {
//     // 1. Identitas Anggota
//     nia?: string;           // kode_anggota
//     kode_anggota?: string;  // Alias untuk kompatibilitas
//     kode?: string;          
    
//     // 2. Biodata Pribadi
//     nik?: string;           
//     no_identitas?: string;  // KTP/SIM
//     gender?: string;        
//     kelamin?: string;       // L/P
//     religion?: string;      
//     agama?: string;
...
//     no_hp?: string;
//     address?: string;       
//     alamat?: string;        // domisili_alamat
//     unit?: string;          // Nama Unit PMI

//     // 5. Meta Data
//     source?: string;        // 'AUTO_REGISTER_POSTGRES' (Penanda akun hasil sync)
//     position?: string;      // Jabatan
//     periodStart?: string | number;
//     periodEnd?: string | number;
    
//     // Allow properti lain jika ada tambahan dari backend di masa depan
//     [key: string]: any; 
//   };
// }
...
//   login: (token: string, userData: User) => void;
//   logout: () => void;
//   loading: boolean; // Agar komponen UI bisa menunggu proses cek login selesai
//   isInitialized: boolean;
// }
...
//         // A. Gunakan data dari LocalStorage dulu agar UI instan (Optimistic UI)
//         if (storedUser) {
//           try {
//             setUser(JSON.parse(storedUser));
//           } catch (e) {
...
//         // B. Validasi Token ke Backend & Ambil Data Terbaru (Self-Healing)
//         try {
//           // Panggil API /me untuk mendapatkan data user terbaru, permissions, dan memberData
//           const res = await api('/api/auth/me'); // Pastikan endpoint ini ada di backend route authentication.ts
...
//              localStorage.setItem('user', JSON.stringify(freshUser));
//           } else {
//              throw new Error("Invalid user data structure from backend");
//           }
//         } catch (err) {
//           console.error("Token expired or invalid:", err);
//           logout(); // Auto logout jika token tidak valid/expired
//         }
//       }
...
//     localStorage.setItem('token', newToken);
//     localStorage.setItem('user', JSON.stringify(newUser));
//     // Note: Redirect dilakukan manual di page login agar lebih fleksibel
//   };
...
// // Hook Custom untuk menggunakan Auth di Component lain
// export const useAuth = () => {
//   const context = useContext(AuthContext);
//   if (context === undefined) {
//     throw new Error('useAuth must be used within an AuthProvider');
...
// [PERUBAHAN PENTING: Menambahkan 'export' agar bisa di-import di file lain]
