...
// //   User, Mail, Phone, MapPin, Edit, MessageSquare, 
// //   BookOpen, FileText, Camera, CreditCard, Calendar, 
// //   Heart, Database, ShieldCheck, Map, Building // [BARU] Import Icon Map & Building
// // } from 'lucide-react';
// // import Link from 'next/link';
// // import ProfilePhotoUploader from '@/components/ProfilePhotoUploader'; 
// // import UserProfileForm from '@/app/admin/members/UserProfileForm'; 
...
// //                             {/* [BARU] INFORMASI WILAYAH & UNIT (PENTING UNTUK PROPOSAL) */}
...
// //                             <InfoRow label="Alamat Lengkap" value={user.memberData?.address || user.memberData?.alamat || '-'} icon={<MapPin size={16}/>} />
// //                         </div>
// //                     </div>
// //                 </div>
// //             )}
...
//     User, Mail, Phone, MapPin, Edit, MessageSquare, 
//     BookOpen, FileText, Camera, CreditCard, Calendar, 
//     Heart, Database, ShieldCheck, Map, Building, LogOut 
// } from 'lucide-react';
// import Link from 'next/link';
// import ProfilePhotoUploader from '@/components/ProfilePhotoUploader'; 
// import UserProfileForm from '@/app/admin/members/UserProfileForm'; 

// // [BARU] Import Komponen KTA
// import MemberCard from '@/components/kta/MemberCard'; 

// export default function ProfilePage() {
//     const { user, logout, login } = useAuth(); // Tambah logout dari useAuth
...
//                     <div className="absolute inset-0 bg-[url('/pattern.png')] opacity-10"></div> {/* Pastikan pattern.png ada di public */}
...
//                         {/* === KOLOM KIRI: KTA DIGITAL & PROFIL SINGKAT === */}
//                         <div className="space-y-6">
...
//                                 {/* Avatar */}
//                                 <div className="relative w-32 h-32 mx-auto mb-4 group cursor-pointer" onClick={() => setShowPhotoModal(true)}>
...
//                                     </div>
//                                     {isSynced && (
//                                         <div className="absolute bottom-1 right-1 bg-blue-500 text-white p-1.5 rounded-full border-2 border-white shadow-sm" title="Terverifikasi Pusat">
//                                             <ShieldCheck size={14} />
//                                         </div>
//                                     )}
//                                 </div>
...
//                                 <p className="text-sm text-gray-500 mb-4 font-medium flex items-center justify-center gap-1">
//                                     <Mail size={12}/> {user.email}
//                                 </p>

//                                 {/* Badges Role */}
//                                 <div className="flex flex-wrap justify-center gap-2 mb-6">
//                                     <span className="px-3 py-1 rounded-full text-xs font-bold bg-red-50 text-red-600 border border-red-100 uppercase tracking-wide">
//                                         {user.role}
//                                     </span>
//                                     {user.memberType && (
...
//                                     <button 
//                                         onClick={() => setShowEditProfile(true)}
//                                         className="flex-1 py-2.5 rounded-xl bg-gray-900 text-white text-sm font-bold hover:bg-gray-800 transition-all flex items-center justify-center gap-2"
//                                     >
//                                         <Edit size={16}/> Edit Profil
//                                     </button>
//                                     <button 
//                                         onClick={logout} 
//                                         className="py-2.5 px-4 rounded-xl border border-gray-200 text-red-600 hover:bg-red-50 transition-all" 
//                                         title="Keluar"
//                                     >
//                                         <LogOut size={18}/>
//                                     </button>
...
//                                             onClick={() => setActiveTab(tab.id as any)}
//                                             className={`
//                                                 flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl transition-all duration-300 ease-out group min-w-max text-sm font-bold
//                                                 ${activeTab === tab.id 
...
//                                             `}
//                                         >
//                                             <tab.icon size={16} className={activeTab === tab.id ? 'text-red-400' : 'text-gray-400'} />
//                                             {tab.label}
//                                         </button>
//                                     ))}
//                                 </div>
...
import { useAuth } from '@/lib/AuthProvider'; // Bukan '@/hooks/useAuth'
import MemberCard from '@/components/kta/MemberCard';
import { 
    User, Mail, Phone, MapPin, 
    CreditCard, Calendar, Heart, 
    ShieldCheck, Building, Download 
} from 'lucide-react';
...
export default function UserProfilePage() {
    ...
    if (!user) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50">
                <p className="text-gray-500">Silahkan login untuk melihat profil.</p>
            </div>
        );
    }

    // 3. Data Preparation
    // Pastikan typescript tidak error dengan akses properti dinamis
        ...
        <div className="min-h-screen bg-gray-50 pb-20 font-sans">
            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6 -mt-20 relative z-10">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    {/* === KOLOM KIRI: KARTU ANGGOTA (4 Kolom) === */}
                    <div className="lg:col-span-4 space-y-6">
                        <div className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden p-6 flex flex-col items-center">
                            <div className="flex items-center gap-2 mb-6 w-full border-b border-gray-50 pb-4">
                                <div className="p-2 bg-red-100 text-red-600 rounded-lg">
                                    <CreditCard size={20}/>
                                </div>
                                <div>
                                    <h3 className="font-bold text-gray-800 text-sm uppercase tracking-wide">KTA Digital</h3>
                                    <p className="text-xs text-gray-400">Kartu Tanda Anggota Resmi</p>
                                </div>
                            </div>
                            ...
                            <button 
                                ...
                                className="mt-8 w-full py-3 bg-gray-900 text-white rounded-xl text-sm font-bold hover:bg-gray-800 transition-colors shadow-lg flex items-center justify-center gap-2"
                            >
                    ...
                    {/* === KOLOM KANAN: DETAIL DATA DIRI (8 Kolom) === */}
                    <div className="lg:col-span-8 space-y-6">
                        <div className="bg-white rounded-3xl shadow-sm border border-gray-200 p-6 md:p-8">
                            ...
                            <div className="flex items-center justify-between mb-8">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-gray-500 text-sm">Informasi data diri Anda yang terdaftar.</p>
                                    </div>
                                </div>
                                
                                <span className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-green-50 text-green-700 text-xs font-bold border border-green-100">
                                    <ShieldCheck size={14}/>
                                    Status: Aktif
                                </span>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                                ...
                                <div className="space-y-6">
                                    <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest border-b pb-2">Identitas Utama</h4>
                                    
                                    <InfoItem label="Nama Lengkap" value={userData.name} icon={<User size={16}/>} />
                                    <InfoItem label="Email" value={userData.email} icon={<Mail size={16}/>} />
                                    <InfoItem label="Nomor Induk Anggota (NIA)" value={memberData.nia || memberData.kode_anggota || '-'} icon={<CreditCard size={16}/>} />
                                    <InfoItem label="No. Handphone" value={memberData.phone || memberData.no_hp || '-'} icon={<Phone size={16}/>} />
                                </div>

                                {/* Group 2: Detail Personal */}
                                <div className="space-y-6">
                                    <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest border-b pb-2">Detail Personal</h4>
                                    
                                    <InfoItem 
                                        label="Tempat, Tanggal Lahir" 
                                        value={`${memberData.tempat_lahir || '-'}, ${formatDate(memberData.tanggal_lahir)}`} 
                                        icon={<Calendar size={16}/>} 
                                    />
                                    <InfoItem label="Jenis Kelamin" value={memberData.kelamin || '-'} icon={<User size={16}/>} />
                                    <InfoItem label="Golongan Darah" value={memberData.golongan_darah || '-'} icon={<Heart size={16}/>} />
                                    <InfoItem label="Agama" value={memberData.agama || '-'} icon={<ShieldCheck size={16}/>} />
                                </div>
                            </div>

                            {/* Group 3: Lokasi & Unit (Full Width) */}
                            <div className="mt-8 pt-8 border-t border-dashed border-gray-200">
                                <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-6">Lokasi & Penugasan</h4>
                                <div className="bg-gray-50 rounded-2xl p-5 grid grid-cols-1 md:grid-cols-2 gap-6 border border-gray-100">
                                    <InfoItem label="Unit PMI" value={memberData.unit || 'PMI PUSAT'} icon={<Building size={16}/>} />
                                    <InfoItem label="Provinsi" value={userData.province || '-'} icon={<MapPin size={16}/>} />
                                    <div className="md:col-span-2">
                                        <InfoItem label="Alamat Lengkap" value={memberData.alamat || memberData.address || '-'} icon={<MapPin size={16}/>} />
                                    </div>
                                </div>
                            </div>
...
const InfoItem = ({ label, value, icon }: { label: string, value: string, icon: any }) => (
    <div className="flex gap-4 group">
        <div className="mt-1 min-w-[32px] h-8 flex items-center justify-center rounded-lg bg-white border border-gray-200 text-gray-400 group-hover:text-red-600 group-hover:border-red-100 transition-colors shadow-sm">
            {icon}
        </div>
        <div className="flex-1">
