...
export default function AdminCoursesPage() {
  ...
  const fetchCourses = async () => {
    ...
    try {
        ...
        const res = await api(`/api/courses?${query}`);
        console.log(`Fetch [${activeMainTab}] Result:`, res); // Cek console browser

        // [FIX] Hapus filter client-side yang ketat. 
        // Kita percaya backend sudah mengirim data yang benar sesuai parameter 'status'.
        // Jika data muncul tapi statusnya 'draft', kita bisa melihatnya di tabel.
        let data = res.courses || [];
        setCourses(data);
  ...
  const fetchFacilitators = async () => {
      try {
          // [FIX] Ignore 404 error agar tidak mengganggu console
  ...
  const handlePublish = async (course: any) => {
      const newStatus = course.status === 'published' ? 'draft' : 'published';
      const action = newStatus === 'published' ? 'Menerbitkan' : 'Menarik kembali';
      if(!confirm(`${action} pelatihan ini?`)) return;
      try { await api(`/api/courses/${course._id}`, { method: 'PATCH', body: { status: newStatus } }); fetchCourses(); } catch (e: any) { alert(e.message); }
  };
