...
export default function AdminCoursesPage() {
    ...
    return (
        <div className="p-6 min-h-screen bg-gray-50 pb-20">
                ...
                {loading ? (
                    ...
                    <>
                        ...
                        {activeTab === 'management' && (
                            <ManagementTable 
                                courses={courses}
                                // onEdit membuka Modal Form, bukan redirect
                                onEdit={handleEditInfo} 
                                onDelete={handleDelete}
                                onPublish={handlePublishToggle}
                                onBroadcast={() => {}}
                                // [FIXED] onViewHistory sekarang membuka Modal Detail/History, BUKAN redirect ke editor
                                onViewHistory={handleViewDetail}
                                currentUser={currentUser}
                            />
                        )}
            ...
            {/* 2. Modal Review Detail (Untuk Admin Approve/Reject ATAU Histori) */}
            {isDetailModalOpen && selectedCourse && (
                <CourseProposalDetailModal 
                    course={selectedCourse}
                    onClose={() => { setIsDetailModalOpen(false); setSelectedCourse(null); }}
                    ...
                    currentUser={currentUser}
                    refreshData={fetchCourses}
                    // ReadOnly jika di tab management, kecuali User punya akses khusus (logic di dalam modal)
                    isReadOnly={activeTab === 'management'} 
                />
            )}
