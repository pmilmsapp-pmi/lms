...
// import { useRouter } from 'next/navigation';
// import { api } from '@/lib/api';
// import { Shield, Users, BookOpen, AlertCircle, MapPin, Settings, CheckCircle, FileText, Database } from 'lucide-react';
...
//         // SINKRONISASI: Cek format respon
...
//   if (loading) return <div className="h-screen flex items-center justify-center bg-gray-100"><div className="animate-spin rounded-full h-10 w-10 border-4 border-orange-500 border-t-transparent"></div></div>;
...
//             <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
//                 {(isSuperAdmin || user?.permissions?.includes('manage_users')) && (
//                     <div onClick={() => router.push('/admin/users')} className="bg-white p-6 rounded-2xl border border-gray-200 hover:border-blue-400 hover:shadow-md cursor-pointer transition-all group">
//                         <div className="w-12 h-12 bg-blue-100 text-blue-600 rounded-xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
//                             <Users size={24}/>
//                         </div>
//                         <h3 className="font-bold text-gray-800 text-lg">Kelola Pengguna</h3>
//                         <p className="text-sm text-gray-500 mt-2">Atur hak akses admin wilayah dan verifikasi user.</p>
//                     </div>
//                 )}

//                 {(isSuperAdmin || user?.permissions?.includes('manage_courses')) && (
//                     <div onClick={() => router.push('/admin/courses')} className="bg-white p-6 rounded-2xl border border-gray-200 hover:border-red-400 hover:shadow-md cursor-pointer transition-all group">
//                         <div className="w-12 h-12 bg-red-100 text-red-600 rounded-xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
//                             <BookOpen size={24}/>
//                         </div>
//                         <h3 className="font-bold text-gray-800 text-lg">Kelola Pelatihan</h3>
//                         <p className="text-sm text-gray-500 mt-2">Buat kurikulum dan publikasikan materi baru.</p>
//                     </div>
//                 )}

//                 {/* BARU: Database Center Card */}
//                 {isSuperAdmin && (
//                     <div onClick={() => router.push('/admin/backup')} className="bg-white p-6 rounded-2xl border border-indigo-200 hover:border-indigo-500 hover:shadow-md cursor-pointer transition-all group bg-indigo-50/10">
...
//                             <Database size={24}/>
//                         </div>
//                         <h3 className="font-bold text-gray-800 text-lg">Database Center</h3>
//                         <p className="text-sm text-gray-500 mt-2">Replikasi data ke Supabase dan Backup Sistem ZIP.</p>
//                     </div>
//                 )}
...
//                             <div className="p-3 bg-orange-100 text-orange-600 rounded-lg"><CheckCircle size={20}/></div>
//                             <div>
//                                 <h4 className="font-bold text-gray-800 text-sm">Verifikasi Pendaftaran</h4>
//                             </div>
//                         </div>
//                     )}
                    
//                     {(isSuperAdmin || user?.permissions?.includes('manage_cms_design')) && (
//                         <div onClick={() => router.push('/admin/content')} className="bg-white p-4 rounded-xl border border-gray-200 hover:bg-purple-50 cursor-pointer flex items-center gap-4">
//                             <div className="p-3 bg-purple-100 text-purple-600 rounded-lg"><FileText size={20}/></div>
//                             <div>
//                                 <h4 className="font-bold text-gray-800 text-sm">CMS & Konten Web</h4>
//                             </div>
//                         </div>
//                     )}
//                 </div>
  ...
  useEffect(() => {
    const initAdminData = async () => {
      try {
        ...
        // Fetch Stats
        // [FIX UTAMA] Gunakan endpoint /stats untuk user agar dapat total ASLI (bukan limit 50)
        const [usersStatsRes, coursesRes] = await Promise.all([
             api('/api/admin/users/stats').catch(() => ({ totalUsers: 0, pendingVerify: 0 })),   
             api('/api/courses').catch(() => ({ courses: [] }))
        ]);
        ...
        setStats({
            totalUsers: usersStatsRes.totalUsers || 0, // Ambil dari endpoint stats
            pendingVerifications: usersStatsRes.pendingVerify || 0, 
            totalCourses: coursesList.length,
            publishedCourses: coursesList.filter((c:any) => c.isPublished).length
        });
