...
export default function FacilitatorDashboard() {
  ...
  useEffect(() => {
    const initData = async () => {
      try {
        ...
        // [SINKRONISASI] Mendukung format data user terbungkus atau tidak
        ...
        console.error("Facilitator Init Error:", err);
        router.push('/login');
      } finally {
        setLoading(false);
      }
  ...
  if (loading) return <div className="h-screen flex items-center justify-center bg-blue-50"><div className="animate-spin rounded-full h-10 w-10 border-4 border-blue-600 border-t-transparent"></div></div>;
