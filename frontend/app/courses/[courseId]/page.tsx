...
// import { useParams, useRouter } from 'next/navigation';
// import { 
//     Clock, BookOpen, Users, PlayCircle, Lock, 
//     Calendar, Award, CheckCircle, ChevronDown, 
//     ChevronUp, Share2, Loader2, Star, Monitor, FileText, UserPlus 
// } from 'lucide-react';
// import { api, getImageUrl } from '@/lib/api';
...
//                 // 2. Default buka modul pertama
//                 if (data.modules && data.modules.length > 0) {
//                     setOpenModules([data.modules[0]._id]);
//                 }

//                 // 3. Get Participants (Untuk Wall of Fame)
//                 try {
//                     const partsRes = await api(`/api/courses/${courseId}/participants`);
//                     const allParts = partsRes.participants || [];
                    
//                     // Filter hanya yang aktif/lulus
//                     const activeParts = allParts.filter((p: any) => p.status === 'active' || p.status === 'approved' || p.status === 'completed');
                    
//                     // Sort: Lulus duluan, lalu progress tertinggi
//                     activeParts.sort((a: any, b: any) => {
...
//                     setTotalParticipants(activeParts.length);
//                 } catch (e) { console.error("Gagal load peserta"); }

//                 // 4. Cek Status Pendaftaran User (Jika login)
//                 const userStr = localStorage.getItem('user');
//                 if (userStr) {
//                     try {
//                         const statusRes = await api(`/api/courses/${courseId}/enrollment-status`).catch(() => ({ isEnrolled: false }));
//                         if (statusRes.isEnrolled) {
//                             setEnrollmentStatus(statusRes.status);
//                         }
//                     } catch (e) { console.error("Guest or error checking status"); }
//                 }
//             } catch (error) {
...
//             alert("Pendaftaran Anda sedang menunggu verifikasi admin.");
//             return;
//         }
...
//             if (course.registrationMode === 'manual') {
//                 alert("Pendaftaran berhasil dikirim! Mohon tunggu verifikasi admin.");
//                 setEnrollmentStatus('pending');
//             } else {
//                 alert("Pendaftaran berhasil! Anda bisa langsung mulai belajar.");
//                 setEnrollmentStatus('active');
//                 router.push(`/learning/${courseId}`);
//             }
//         } catch (e: any) {
//             alert("Gagal daftar: " + (e.message || "Terjadi kesalahan"));
//         } finally {
...
//     if (!course) return <div className="min-h-screen flex items-center justify-center text-gray-500 font-medium">Pelatihan tidak ditemukan.</div>;

//     const totalLessons = course.modules?.reduce((acc: number, m: any) => acc + (m.lessons?.length || 0), 0) || 0;
//     const isFree = course.price === 0;

//     return (
//         <div className="min-h-screen bg-gray-50 pb-24 font-sans relative overflow-x-hidden selection:bg-red-100 selection:text-red-900">
            
//             {/* --- HERO HEADER --- */}
//             <div className="relative bg-[#990000] text-white pt-32 pb-48 lg:pb-64 overflow-hidden">
//                 <div className="absolute inset-0 z-0 opacity-20 mix-blend-overlay">
//                     {course.thumbnailUrl && (
//                         <img src={getImageUrl(course.thumbnailUrl)} className="w-full h-full object-cover filter blur-sm scale-110" alt="bg"/>
//                     )}
//                 </div>
//                 <div className="absolute inset-0 bg-gradient-to-t from-[#990000] via-transparent to-black/30 z-0"></div>

//                 <div className="container max-w-7xl mx-auto px-6 relative z-10">
//                     <div className="max-w-4xl">
//                         <div className="flex flex-wrap gap-3 mb-6 animate-in slide-in-from-bottom-4 fade-in duration-700">
...
//                                 {course.programType === 'training' ? 'Diklat Resmi' : 'Kursus Mandiri'}
//                             </span>
//                             <span className="px-3 py-1 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full text-[10px] font-bold uppercase tracking-wider flex items-center gap-1">
//                                 <Award size={12} className="text-yellow-400"/> {course.category || 'Umum'}
//                             </span>
//                         </div>

//                         <h1 className="text-3xl md:text-5xl lg:text-6xl font-black leading-tight mb-6 drop-shadow-lg animate-in slide-in-from-bottom-6 fade-in duration-700 delay-100">
//                             {course.title}
//                         </h1>

//                         <div className="flex flex-wrap items-center gap-6 text-sm font-medium text-red-100 animate-in slide-in-from-bottom-8 fade-in duration-700 delay-200">
//                             <div className="flex items-center gap-2"><Clock size={18}/> {course.estimatedDuration || 60} Menit</div>
//                             <div className="flex items-center gap-2"><BookOpen size={18}/> {course.totalJp || 0} JP</div>
//                             <div className="flex items-center gap-2"><Users size={18}/> {course.reviewsCount || 0} Peserta</div>
//                             <div className="flex items-center gap-2"><Star size={18} className="text-yellow-400 fill-yellow-400"/> {course.rating || 'New'}</div>
//                         </div>
//                     </div>
//                 </div>
//             </div>
...
//             {/* --- MAIN CONTENT LAYOUT --- */}
//             <div className="container max-w-7xl mx-auto px-4 lg:px-6 relative z-20 -mt-32 lg:-mt-48">
//                 <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
...
//                         {/* 1. About Section */}
//                         <div className="bg-white rounded-3xl p-8 shadow-xl shadow-gray-200/50 border border-gray-100 animate-in slide-in-from-bottom-10 duration-700">
//                             <h3 className="text-xl font-bold text-gray-900 mb-6 flex items-center gap-2">
//                                 <FileText className="text-red-600"/> Deskripsi Program
//                             </h3>
//                             <div 
//                                 className="prose prose-red max-w-none text-gray-600 leading-relaxed font-normal"
...
//                                         <BookOpen className="text-red-600"/> Kurikulum
//                                     </h3>
//                                     <p className="text-sm text-gray-500 mt-1">{course.modules?.length || 0} Modul • {totalLessons} Materi</p>
//                                 </div>
//                                 <button 
//                                     onClick={() => setOpenModules(course.modules.map((m:any) => m._id))} 
...
//                             <div className="space-y-4">
//                                 {course.modules?.filter((m: any) => m.isActive).map((mod: any, idx: number) => (
//                                     <div key={mod._id} className="border border-gray-200 rounded-2xl overflow-hidden transition-all duration-300 hover:border-red-200 hover:shadow-md bg-white">
//                                         <button 
//                                             onClick={() => toggleModule(mod._id)}
//                                             className="w-full flex justify-between items-center p-5 bg-white hover:bg-gray-50 transition-colors text-left"
//                                         >
//                                             <div className="flex items-center gap-4">
//                                                 <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-black transition-colors ${openModules.includes(mod._id) ? 'bg-red-600 text-white shadow-lg shadow-red-200' : 'bg-gray-100 text-gray-500'}`}>
//                                                     {idx + 1}
//                                                 </div>
//                                                 <div>
//                                                     <h4 className="font-bold text-gray-800 text-sm md:text-base">{mod.title}</h4>
//                                                     <p className="text-[10px] text-gray-400 mt-0.5 font-medium uppercase tracking-wide">{mod.lessons?.length || 0} Materi • {mod.jp || 0} JP</p>
//                                                 </div>
//                                             </div>
//                                             {openModules.includes(mod._id) ? <ChevronUp size={20} className="text-red-500"/> : <ChevronDown size={20} className="text-gray-400"/>}
//                                         </button>

//                                         {openModules.includes(mod._id) && (
//                                             <div className="bg-gray-50/50 border-t border-gray-100 divide-y divide-gray-100 animate-in slide-in-from-top-2 duration-300">
//                                                 {mod.lessons?.filter((l: any) => l.isActive).map((les: any) => (
//                                                     <div key={les._id} className="p-4 pl-[4.5rem] flex justify-between items-center group hover:bg-white transition-colors">
//                                                         <div className="flex items-center gap-3">
//                                                             {les.type === 'video_url' ? <PlayCircle size={16} className="text-blue-500"/> : <FileText size={16} className="text-gray-400"/>}
//                                                             <span className="text-sm font-medium text-gray-600 group-hover:text-gray-900 transition-colors line-clamp-1">{les.title}</span>
//                                                         </div>
//                                                         {enrollmentStatus === 'active' ? (
//                                                             <span className="text-[10px] bg-green-100 text-green-700 px-2 py-0.5 rounded font-bold">Akses</span>
//                                                         ) : (
//                                                             les.isPreview ? 
//                                                                 <span className="text-[10px] text-blue-600 font-bold bg-blue-50 px-2 py-0.5 rounded cursor-pointer hover:bg-blue-100">Preview</span> : 
//                                                                 <Lock size={14} className="text-gray-300"/>
//                                                         )}
//                                                     </div>
//                                                 ))}
//                                                 {(!mod.lessons || mod.lessons.length === 0) && (
//                                                     <div className="p-4 text-center text-xs text-gray-400 italic">Belum ada materi di modul ini.</div>
//                                                 )}
//                                             </div>
//                                         )}
//                                     </div>
...
//                             <div className="bg-white rounded-3xl p-8 shadow-xl shadow-gray-200/50 border border-gray-100">
//                                 <h3 className="text-xl font-bold text-gray-900 mb-6 flex items-center gap-2">
//                                     <Users className="text-red-600"/> Tim Fasilitator
//                                 </h3>
//                                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
//                                     {course.facilitatorIds.map((fac: any) => (
//                                         <div key={fac._id || fac} className="flex items-center gap-4 p-4 rounded-2xl border border-gray-100 bg-gray-50/50 hover:bg-white hover:shadow-md transition-all">
//                                             <div className="w-12 h-12 rounded-full bg-gray-200 overflow-hidden border-2 border-white shadow-sm shrink-0">
//                                                 {fac.avatarUrl ? (
//                                                     <img src={getImageUrl(fac.avatarUrl)} alt={fac.name} className="w-full h-full object-cover"/>
//                                                 ) : (
//                                                     <div className="w-full h-full flex items-center justify-center text-gray-400 font-bold text-lg bg-white">{fac.name?.charAt(0)}</div>
//                                                 )}
//                                             </div>
//                                             <div>
//                                                 <h4 className="font-bold text-gray-900 text-sm">{fac.name}</h4>
//                                                 <p className="text-[10px] text-blue-600 font-bold uppercase tracking-wide mt-0.5">Fasilitator PMI</p>
//                                             </div>
//                                         </div>
//                                     ))}
//                                 </div>
...
//                             {/* Course Card */}
//                             <div className="bg-white rounded-[32px] shadow-2xl border border-gray-200 overflow-hidden relative group">
...
//                                         <div className="w-14 h-14 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center shadow-2xl border border-white/50 group-hover:scale-110 transition-transform">
...
//                                         <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-1">Investasi Program</p>
//                                         <div className="flex items-baseline gap-2">
//                                             {isFree ? (
//                                                 <h2 className="text-4xl font-black text-green-600">GRATIS</h2>
//                                             ) : (
//                                                 <h2 className="text-3xl font-black text-gray-900">Rp {course.price.toLocaleString('id-ID')}</h2>
//                                             )}
//                                         </div>
//                                     </div>
...
//                                             <span className="flex items-center gap-2 text-blue-800 font-bold"><Calendar size={16}/> Pendaftaran</span>
//                                             <span className="text-xs font-bold text-gray-600 bg-white px-2 py-1 rounded border">
//                                                 {course.registrationPeriod?.isForever ? 'Selamanya' : `${formatDate(course.registrationPeriod?.startDate)} - ${formatDate(course.registrationPeriod?.endDate)}`}
//                                             </span>
//                                         </div>
//                                         <div className="flex justify-between items-center">
//                                             <span className="flex items-center gap-2 text-blue-800 font-bold"><Clock size={16}/> Pelaksanaan</span>
//                                             <span className="text-xs font-bold text-gray-600 bg-white px-2 py-1 rounded border">
//                                                 {course.executionPeriod?.isForever ? 'Fleksibel' : `${formatDate(course.executionPeriod?.startDate)} - ${formatDate(course.executionPeriod?.endDate)}`}
//                                             </span>
//                                         </div>
//                                     </div>
...
//                                                 className="w-full py-4 rounded-xl bg-green-600 text-white font-black text-sm shadow-lg shadow-green-200 hover:bg-green-700 hover:shadow-xl hover:-translate-y-1 transition-all flex items-center justify-center gap-2 uppercase tracking-wide"
//                                             >
//                                                 Lanjut Belajar <CheckCircle size={18}/>
//                                             </button>
//                                         ) : enrollmentStatus === 'pending' ? (
//                                             <div className="w-full py-4 rounded-xl bg-yellow-50 text-yellow-700 font-bold text-sm text-center border-2 border-yellow-100 flex items-center justify-center gap-2 uppercase tracking-wide cursor-not-allowed">
//                                                 <Loader2 size={16} className="animate-spin"/> Menunggu Verifikasi
//                                             </div>
//                                         ) : (
//                                             <button 
//                                                 onClick={handleEnroll}
//                                                 disabled={isEnrolling}
//                                                 className="w-full py-4 rounded-xl bg-[#990000] text-white font-black text-sm shadow-lg shadow-red-200 hover:bg-[#7f0000] hover:shadow-xl hover:-translate-y-1 transition-all flex items-center justify-center gap-2 uppercase tracking-wide disabled:opacity-70 disabled:cursor-not-allowed"
//                                             >
//                                                 {isEnrolling ? <Loader2 className="animate-spin" size={18}/> : 'Daftar Sekarang'}
//                                             </button>
//                                         )}
                                        
//                                         <button className="w-full py-3 rounded-xl border-2 border-gray-100 text-gray-600 font-bold text-xs hover:bg-gray-50 transition-colors flex items-center justify-center gap-2 uppercase tracking-wider">
//                                             <Share2 size={16}/> Bagikan
//                                         </button>
//                                     </div>

//                                     <div>
//                                         <h4 className="font-bold text-xs text-gray-900 uppercase tracking-wider mb-3">Fasilitas yang didapat:</h4>
//                                         <ul className="space-y-2.5">
//                                             <li className="flex items-start gap-3 text-sm text-gray-600"><CheckCircle size={16} className="text-green-500 mt-0.5 shrink-0"/> <span>Akses {course.modules?.length} Modul Materi</span></li>
//                                             <li className="flex items-start gap-3 text-sm text-gray-600"><CheckCircle size={16} className="text-green-500 mt-0.5 shrink-0"/> <span>Akses Selamanya</span></li>
//                                             {course.hasCertificate && <li className="flex items-start gap-3 text-sm text-gray-600"><CheckCircle size={16} className="text-green-500 mt-0.5 shrink-0"/> <span>Sertifikat Digital Resmi</span></li>}
//                                             {course.facilities?.map((fac: string, i: number) => (
//                                                 <li key={i} className="flex items-start gap-3 text-sm text-gray-600"><CheckCircle size={16} className="text-green-500 mt-0.5 shrink-0"/> <span>{fac}</span></li>
//                                             ))}
//                                         </ul>
//                                     </div>
//                                 </div>
...
//                             {/* [NEW] WALL OF FAME (DAFTAR PESERTA) */}
//                             <div className="bg-white rounded-[32px] border border-gray-200 p-6 shadow-lg">
//                                 <div className="flex justify-between items-center mb-4">
//                                     <h4 className="font-black text-gray-900 text-xs uppercase tracking-[0.2em] flex items-center gap-2">
//                                         <UserPlus size={16} className="text-red-600"/> Peserta ({totalParticipants})
//                                     </h4>
//                                     <span className="text-[10px] text-gray-400">Top 5 Recent</span>
//                                 </div>
                                
//                                 <div className="space-y-3">
//                                     {participants.length > 0 ? participants.slice(0, 5).map((p, i) => (
//                                         <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-gray-50 border border-gray-100 hover:bg-white hover:shadow-md transition-all">
//                                             <div className="flex items-center gap-3">
//                                                 <div className="w-8 h-8 rounded-full bg-gray-200 overflow-hidden border border-white shadow-sm">
//                                                     {p.user?.avatarUrl ? (
//                                                         <img src={getImageUrl(p.user.avatarUrl)} alt={p.user?.name} className="w-full h-full object-cover"/>
//                                                     ) : (
//                                                         <div className="w-full h-full bg-blue-100 flex items-center justify-center font-bold text-blue-500 text-xs">
//                                                             {p.user?.name?.charAt(0) || 'U'}
//                                                         </div>
//                                                     )}
//                                                 </div>
//                                                 <div>
//                                                     <p className="text-xs font-bold text-gray-700 truncate max-w-[120px]">{p.user?.name || 'Peserta'}</p>
//                                                     <p className="text-[9px] text-gray-400">Bergabung {new Date(p.joinedAt || p.createdAt).toLocaleDateString()}</p>
//                                                 </div>
//                                             </div>
//                                             {p.isCompleted ? (
//                                                 <span className="text-[9px] font-black bg-green-100 text-green-700 px-2 py-1 rounded-md border border-green-200">LULUS</span>
//                                             ) : (
//                                                 <span className="text-[9px] font-bold bg-gray-200 text-gray-500 px-2 py-1 rounded-md">AKTIF</span>
//                                             )}
//                                         </div>
//                                     )) : (
//                                         <p className="text-center text-xs text-gray-400 italic py-4">Belum ada peserta aktif.</p>
//                                     )}
//                                 </div>
//                             </div>
...
//                         <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 shadow-[0_-5px_20px_rgba(0,0,0,0.1)] z-50 flex items-center justify-between">
//                             <div>
//                                 <p className="text-[10px] text-gray-400 font-bold uppercase">Investasi</p>
//                                 <p className="font-black text-gray-900">{isFree ? 'GRATIS' : `Rp ${course.price.toLocaleString('id-ID')}`}</p>
//                             </div>
//                             {enrollmentStatus === 'active' ? (
//                                 <button onClick={() => router.push(`/learning/${courseId}`)} className="bg-green-600 text-white px-6 py-2.5 rounded-lg font-bold text-sm shadow-md">Lanjut</button>
//                             ) : (
//                                 <button onClick={handleEnroll} disabled={isEnrolling} className="bg-[#990000] text-white px-6 py-2.5 rounded-lg font-bold text-sm shadow-md disabled:opacity-50">
//                                     {isEnrolling ? '...' : 'Daftar'}
//                                 </button>
//                             )}
//                         </div>
...
export default function CourseLandingPage() {
    ...
    const handleEnroll = async () => {
        ...
        // [FIX PATH] Redirect ke halaman Play yang benar
        if (enrollmentStatus === 'active') {
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    <div className="lg:col-span-4 space-y-6">
                            ...
                            <div className="bg-white rounded-[32px] shadow-2xl border border-gray-200 overflow-hidden relative group">
                                ...
                                <div className="p-6">
                                    ...
                                    <div className="space-y-3 mb-6">
                                        {enrollmentStatus === 'active' ? (
                                            <button 
                                                // [FIX PATH] Redirect ke /courses/[id]/play
                        ...
                        <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 shadow-[0_-5px_20px_rgba(0,0,0,0.1)] z-50 flex items-center justify-between">
                            ...
                            {enrollmentStatus === 'active' ? (
                                // [FIX PATH] Redirect ke /courses/[id]/play
                                <button onClick={() => router.push(`/courses/${courseId}/play`)} className="bg-green-600 text-white px-6 py-2.5 rounded-lg font-bold text-sm shadow-md">Lanjut</button>
                            ) : (
                                <button onClick={handleEnroll} disabled={isEnrolling} className="bg-[#990000] text-white px-6 py-2.5 rounded-lg font-bold text-sm shadow-md disabled:opacity-50">
                                    {isEnrolling ? '...' : 'Daftar'}
                                </button>
